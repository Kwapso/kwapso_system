...
const RETIRED_SINCE_THE_CAPTURE: Record<string, string> = {
  ...
  account_rate:
    "what a CLIENT is charged, retired ONE HOUR LATER the same day at the same person's second ruling: \"The whole account rates also killed it.\" The line above said `account_rate` was untouched; it was true for an hour. Its table (`account_rates`, dropped by team migration 0078), its four doors and its three named tools went with it, so this is a SECOND change to the same published contract — `set_record_active` now names two fewer records than the capture did. THE TWO LINES TOGETHER ARE THE POINT OF THIS SHAPE: the matrix above is evidence of what twenty-one tools answered, and it is still exactly that, because neither ruling was allowed to edit it.",
  staff_certificate:
    "the team's credential register, retired on 14 Sep 2026 at the client's ruling — \"kill the whole certificate module everywhere.\" Its table (`staff_certificates`, dropped by team migration 0090), its five doors and every tool that named it (`list_staff_certificates`, `create_staff_certificate`, `update_staff_certificate`, `export_certificates_csv`, and this record's own `set_staff_certificate_active`) went with it, so `set_record_active` now names one fewer record than the capture did. A THIRD change to the same published contract, for the same reason the two above are: the door this record routed to does not exist any more.",
}
