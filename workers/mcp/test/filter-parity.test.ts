...
const TOOLLESS_DOORS: Record<string, string> = {
  ...
  "POST /api/tenancy/accounts/archived":
    "0117, 22 Sep 2026 — her ARCHIVED, a second and stronger state than `active`/`deactivated_at`, deliberately NOT folded into `set_record_active`'s generic `active` axis above (this door's own sibling, `/accounts/active`, is one of that tool's 21 routed record kinds): archived is an accounts-only fact, independent of active/inactive, and widening the shared generic tool's shape to carry a SECOND boolean for one record kind out of 21 is a bigger change than this round's own scope. `setAccountActive`/`/accounts/active` itself has no accounts-specific tool of its own either — only the generic one — so this door staying off the machine surface for now matches its sibling's own posture rather than adding an inconsistency. DELETE THIS LINE if the machine surface is ever asked to archive/unarchive an account — a small, dedicated `set_account_archived` tool (id + archived boolean) is the honest shape then, not a stretched `set_record_active`.",
  "GET /api/tenancy/accounts/dashboard":
    "23 Sep 2026, the Accounts screen's own Dashboard tab (her ruling: three surviving questions over the active company book — how many, how many countries, by country, by arrival month). It takes no query parameter at all, so R19 has nothing to expose or forward here; the whole door exists to hand ONE screen three small pre-grouped pictures in a single round trip, the same reasoning `GET /api/content/knowledge/shape` gives for staying off this surface. `list_accounts` already answers a machine caller's most likely question, an exact company count, through its own `entityTotal` — the by-country and by-arrival-month GROUPINGS have no machine equivalent today. DELETE THIS LINE if a developer asks for 'how many active companies per country' or 'accounts by arrival month' as a capability — that is a real, small counting tool over the same active-company fence, and a different one from this door.",
  "GET /api/tenancy/config/maps":
    "23 Sep 2026, her ruling to build the accounts map with the Google Maps API. This door answers ONE question a browser has and no machine ever does: what script URL should this page load to draw a map. It carries no record, no field and no filter, so there is nothing here for the grammar to expose. It exists at all only because the key must be composed into the URL SERVER side rather than shipped to the page as a value to assemble, which is the same shape `buildGoogleStart` uses for Google sign in. A tool for it would hand a model a credentialed URL it has no use for and every reason not to hold. DELETE THIS LINE if the map ever gains a machine readable answer of its own, for example where an account sits, which is a field on the account and belongs in the grammar rather than here.",
  ...
  "POST /api/content/knowledge/glossary/seed":
    "a one-time UI bootstrap, not a machine capability: it writes this BUILD's own 54 shipped words (shared/glossary-seed.ts), which is a fact about the app's release rather than something a caller ever chooses or names. The capability itself, adding a word to the glossary, is already a tool, add_glossary_word, and a machine that wants the whole starting set can call it 54 times. Idempotent either way, so nothing is lost by leaving the bulk seed off this surface.",
  ...
  "GET /api/content/help/stages":
    "ONE TICKET'S OWN LADDER, drawn as a strip above its Activity feed: which rungs, in what order, and how long it sat on each (team migration 0066). It is a SHAPE FOR A SCREEN rather than a capability, and the argument is the dashboard's one door up. The rows are transitions, and every question a machine actually asks of them is an aggregate across the BACKLOG — how long does a ticket sit in triage, how often does a kind come back out of resolved — which this door cannot answer at all: it takes one id and answers about one request. The per-ticket facts a machine can act on are already on the ticket itself through `get_help_ticket` (`status`, `createdAt`, `resolvedAt`), and the durations are arithmetic over those. When the aggregate question is answered it will be answered where the other groupings are, on the tickets module through `query_records`, not by handing a model a list of one ticket's rungs and asking it to add up the gaps. DELETE THIS LINE the day the stage history becomes a groupable field of that module.",

  "GET /api/content/help/rating":
    "HOW WE DID, ACCORDING TO THE CLIENT, on one ticket (team migration 0067) — and the reason it is silent here is that the agency-side SHAPE of this read is still an open design question. The owner ruled on the storage and on the portal's own control; she explicitly did not ask for an agency dashboard yet, and putting a tool on the door first would publish a per-ticket answer shape that the aggregate ('is this client happy') is unlikely to keep. The fact is not hidden from the agency in the meantime: rating a ticket writes an activity row, so what a client said is already readable on the ticket's own history, which is a surface this token can reach. DELETE THIS LINE when the sentiment read has an agreed shape — a tool then belongs on whatever door answers 'how did we do' across a client, not on this one.",

  "POST /api/content/help/rating":
    "A CLIENT SAYING HOW WE DID IS NOT A THING A MACHINE MAY SAY FOR THEM. This is the same refusal the two client-portal standing doors above carry, in its strongest form: the door exists so the company that paid for the work can give a verdict on it, the row stamps WHO gave it, and a token acting as a staff member could only ever write a staff member's guess at somebody else's opinion into the column the answer to 'is this client happy' is computed from. That is not a narrower capability, it is a corrupted one — and unlike a wrong status it can never be spotted afterwards, because a sincere rating and an invented one are the same row. A staff member who takes the verdict down a phone call types it in the app, under their own name, where the record says a person did it.",

  "POST /api/content/work-logs/update":
    "CORRECTING somebody's hours, which is a different act from logging your own and is gated a step higher (work:update rather than work:create). The machine surface can start a timer, stop one, write time down and answer for a runaway — everything a person does about their OWN time — but a timesheet correction is the one write in this module that changes a number after the fact, and time is the record here that turns into money. It leaves a trail either way (lib/work-logs editWorkLog writes one); the reason it has no tool is that nobody should be able to say 'make last Tuesday four hours' to an assistant and have it happen. A person opens the row.",
...
const NARROWED_BODY_FIELDS: Record<string, string> = {
  ...
  "POST /api/tenancy/apps/attachments::kind":
    "the ticket and story doors' own line, a worker along (T3850): the door takes 'file' or 'link' and this surface can only ever send one of them. `add_app_link` FORWARDS `kind: \"link\"` as a constant, so the door's whole contract is honoured — what is withheld is the ARGUMENT.",
  "POST /api/tenancy/apps/attachments::fileDataUrl":
    "bytes, not prose — the same objection as every base64 field on this list. An app's file is a base64 data URL up to 10 MB (TICKET_FILE_MAX_BYTES, shared with the ticket and story doors), around 14 million characters of argument on a surface whose whole ANSWER is capped at 400,000. The LINK half is exposed in full; a file is attached from a screen by the person holding it.",
  "POST /api/tenancy/apps/attachments/update::fileDataUrl":
    "the add door's line, on the door that FIXES what that one attached: bytes, not prose, for the identical reason the story twin gives. `update_app_attachment` renames anything, file or link, and repoints a link, all exposed in full; only the replacement bytes are withheld.",
  ...
  "POST /api/data-ops/agent/chat::attachments":
    "the same bytes-not-prose objection as every base64 field on this list, one size class down: up to 3 files of 4 MB each, around 16 MB of argument on a surface whose whole ANSWER is capped at 400,000 — and one more angle on top of it, from the SAME reasoning as the to-do attachment two lines up. `assistant a1` (18 Sep 2026) is explicitly a picked file shown as a tile the person can see and remove before pressing send, read for that one conversation and never stored anywhere — a screen a headless client does not have. A machine attaching bytes here would not be relaying something a person picked, it would be INVENTING the one thing this feature's whole design promises is a person's own choice. Nothing is lost: a token holder that already has the file's words (it read them from somewhere) can simply say them to agent_chat as message text, which every tool on this surface already carries in full.",
}
