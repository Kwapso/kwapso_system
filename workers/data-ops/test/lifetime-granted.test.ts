...
/** The record every grant must now carry (R: a top-up leaves a trace). Its own
 * proof lives in `grant-leaves-a-record.test.ts`; here it is just the argument. */
const GRANT = { actor: "owner-key", requestId: "01JREQUEST" } as const
 ...
 * real binding — `prepare().bind()`, then `.run()`/`.first()`, and `batch()`,
 * which the grant uses so the money and its audit row commit together — so this
 * proves the lib's own call, not an agreement between two stubs. */
...
describe("agent_credits.lifetime_granted has a reader", () => {
  it("grantCredits reads the running total back and returns it beside the balance", async () => {
    ...
    const out = await grantCredits(env, "01JTEAM", 20, GRANT)
  ...
  it("answers honestly when the row is missing rather than inventing a total", async () => {
    ...
    expect(await grantCredits(env, "01JTEAM", 20, GRANT)).toEqual({ balance: 0, lifetimeGranted: 0 })
  })
