...
`,
  },
  {
    ...
    // whether it may run.
    version: "0084_the_team_says_which_automations_are_off",
    sql: `
...
`,
  },
  {
    ...
    //
    // RENUMBERED FROM 0093 TO 0094 during the 15 Sep 2026 merge with the ticket
    // types cut (also opened as 0093 on a parallel branch): his lands first in
    // the ledger, ours after it, so the version string moves and nothing else
    // does — see the merge commit for the coordinator's staging `_migrations`
    // rewrite.
    version: "0094_story_type_and_category",
    sql: `
...
`,
  },
  {
    ...
    version: "0095_everyones_stories",
    sql: `
...
`,
  },
  {
    ...
    // the old name.
    //
    // The second is 0095's own shape, one switch along: `all_inputs`, off
    ...
    version: "0096_todos_permission_renamed_inputs",
    sql: `
...
`,
  },
  {
    ...
    // with `ref IS NULL`.
    version: "0101_every_ticket_and_story_gets_a_number",
...
/** 0101's SQL — see the migration's own header, above in TEAM_MIGRATIONS, for
...
INSERT INTO _numbering_0101 (entity_table, row_id, becomes)
SELECT '${table}', id, ${canonicalRefSql(kind, "n")} FROM seated;

UPDATE ${table}
   SET ref = (SELECT becomes FROM _numbering_0101
               WHERE entity_table = '${table}' AND row_id = ${table}.id)
 WHERE ref IS NULL
   AND id IN (SELECT row_id FROM _numbering_0101 WHERE entity_table = '${table}');
...
CREATE TABLE IF NOT EXISTS _numbering_0101 (
  entity_table TEXT NOT NULL,
  row_id TEXT NOT NULL,
  becomes TEXT NOT NULL,
  PRIMARY KEY (entity_table, row_id)
);
DELETE FROM _numbering_0101;
${perTable}
DROP TABLE _numbering_0101;
`
}
