...
- **`web/components/` has no top-level files**: one folder per MODULE (`tickets/`, `work/`, `accounts/`, `apps/`, `process/`, `money/`, `team/`, `knowledge/`, `meetings/`, `choices/`) or per KIND (`shell/`, `records/`, `deep-link/`, `assistant/`, `screens/`). A new component joins its module's folder; a new module gets one. **No basename changed** in the 7 Sep 2026 fold of 128 flat files — every census that finds a screen by NAME still finds it — and every law that reads components off disk goes through `sourceFiles()`, which RECURSES by default, so the before/after counts are identical (148 → 148, and `appFiles()` 337 → 337). `web/test/source-scan.test.ts` now asserts the walk reaches more than a hundred nested files, because a walk that stopped at the top level would enforce the UI laws on nothing and report success for all of it. Name a component by BASENAME through the walk, never by folder path (UI-CONVENTIONS.md §1).
