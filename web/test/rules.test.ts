...
describe("RULES — the laws of the base", () => {
  ...
  it("every enforced law has a known check", () => {
    ...
    "pinned-toolbar", // R63: the seam guard + the toolbar-owner census (R53's own list, plus the portal's door-searched rows) + the nobody-hand-rolls-the-offset scan + the four declarations of --pinned-chrome-h, above
      ...
      "id-chip-is-black", // R96: web/test/id-chip-is-black.test.ts, every bare {x.ref} inside a non-inverse <Badge>, plus every standalone field("ref", <any label>) column, over web/ + web-portal/ + shared/web/, or named in ID_CHIP_EXEMPT
      ...
      "empty-register-on-the-page", // R103: web/test/empty-register-on-the-page.test.ts, every <CollectionEmptyState>/<PortalEmpty> ancestor-chain census for a painted Card/bg-surface-panel/bg-card box, plus every <EmptyGatedPanel surface="boxed">, over web/components + web-portal/components + shared/web/screen-engine, or named in EMPTY_REGISTER_EXEMPT
      "people-as-chips", // R104: web/test/people-as-chips.test.ts, every <PersonCard> inside a record section whose ancestor chain reaches a <Card> of any variant, over web/components/tickets + work + apps + accounts, or its whole file named in PEOPLE_AS_CHIPS_EXEMPT
      ...
      "filters-open-as-an-overlay", // R110: web/test/filters-open-as-an-overlay.test.tsx — filterOverlayForm/FACET_SPAN/FILTER_POPOVER_BUDGET asserted as a pure function AND re-asked of the rendered dialog's data-form, the toolbar's in-flow markup proved byte-identical at all three widths with the overlay portaled outside column/pin-box/container, the overlay's own dismissal + focus + two foot controls, and four censuses over web/ + web-portal/ + shared/web/ (no toolbarPanel, no destructured panel out of useFilterBar, every toolbar-row-track carrying FILTER_ANCHOR_ATTR on the same element, and FilterOverlay reached through the kit with the app-side twin asserted absent)
      "photo-beats-initials", // R111: web/test/photo-beats-initials.test.ts — every <RecordMark shape="round"> and <PersonCard> under web/components carrying no picture= is a finding, read through the shared comment stripper, or named in PHOTO_UNREACHABLE keyed by {file, contains} and rot-checked both ways, plus a synthetic-sample tripwire so the parser cannot pass by matching nothing
      "archived-hides-its-children", // R112: workers/content/test/archived-hides-its-children.test.ts — against a real node:sqlite database running the real team migrations: which tables carry the archive quartet read off pragma_table_info against ARCHIVABLE, the deactivatable tables asserted to hide nobody's children, an archived ticket's story and to-do gone from every list and count with the no-ticket controls kept and unarchive restoring both, the record map's ARCHIVED_PARENT_HIDES, the Archive view still opening it, resolveAccountScope refusing a contact of an archived account while staff stay staff, and each knowledge reader asserted to RETIRE on its parent's aliased column; PLUS workers/tenancy/test/archiving-cascades.test.ts for the 24 Sep 2026 amendment (cascading rather than filtering): the owning graph as data with her four rulings as its clauses, work_logs asserted absent as a negative, a four-level chain archived and restored, the marker walking past a child archived on its own merits, idempotence both ways, the 409 parent_archived refusal, the portal corridor still refusing a cascaded contact, and the honest D1 partial-cascade recovery
    ])
