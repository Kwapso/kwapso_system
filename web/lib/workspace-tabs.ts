...
export function closeAllTabs(keepPath: string): void {
  ...
  if (!keep) return
  // PRUNE THE CANONICAL-KEY MAP FOR EVERY TAB THIS REMOVES — the same
  // bookkeeping `closeTab` does one path at a time, done here for all of them
  // at once. Left stale, a closed tab's canonical key would keep pointing at
  // a path no longer in `tabs`; `visitTrail`'s own `put` overwrites a stale
  // entry the next time that key is visited, so this is not a correctness
  // bug, but leaving map rot behind a "close" is not this file's habit.
  for (const [key, path] of canonicalKeys) {
    if (path !== keepPath) canonicalKeys.delete(key)
  }
  tabs = [keep]
  recency = [keepPath]
  activePath = keepPath
