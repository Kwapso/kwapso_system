...
export const SHARED_TOOLS: SharedTool[] = [
  ...
  // refuses them both with the same sentence.
  {
    ...
    buildQuery: (i) => {
      ...
      // The door reads both flags as the literal "1" (R20: a query value is
      // checked where it sits), so the only truthy spelling this surface sends
      // is that one, for either.
      if (i.compose === true) q.push("compose=1")
      if (i.read === true) q.push("read=1")
      return `?${q.join("&")}`
    },
