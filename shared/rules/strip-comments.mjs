// @ts-check
// ^ NOT decoration. `allowJs` is on in web/tsconfig.json, and this file is named
// in its `include`, so `npm run check` typechecks the JSDoc below exactly as it
// typechecks a .ts — the types here are verified against the code, not just
// written beside it. (The TypeScript CALLERS read `strip-comments.d.mts`
// instead; a worker's tsconfig has `allowJs` off and will not open a .mjs at
// all. That file's header has the measurement.)
// THE COMMENT STRIPPER. PLAIN JAVASCRIPT, ON PURPOSE.
//
// This is the tokeniser every law in this repo reads source through, and it is
// a `.mjs` rather than a `.ts` for ONE reason, which is worth stating at length
// because the next person to look at it will assume it was an oversight.
//
// TWO WORLDS RUN THIS CODE, and only one of them compiles TypeScript.
//
//   · The LAWS run under vitest, which transpiles TypeScript on the way in.
//     They import it through `shared/rules/source-scan.ts`, which re-exports
//     everything below, so nothing on that side changed when this file was
//     split out and nothing on that side needs to know it exists.
//   · `scripts/*.mjs` run under PLAIN NODE, with no build step and no loader —
//     `npm run smoke:portal` is `node scripts/smoke-portal.mjs`, and it is the
//     LAST step of `deploy:staging`.
//
// The second world is why this is not a `.ts`. Node only strips types without a
// flag from 22.18; `package.json` declares `engines.node: ">=22"`. A script on
// the deploy path that imported a `.ts` would therefore work on the machine it
// was written on and fail on a machine the repo says is supported — and it
// would fail at the END of a deploy, after eight workers had already shipped.
// A stripper that both worlds can import without a toolchain is a smaller
// promise than a version bump, so that is the promise this file makes.
//
// WHAT WENT WRONG WHEN THEY WERE NOT THE SAME CODE. `scripts/smoke-portal.mjs`
// hand-rolled the two regexes this tokeniser replaced, and used them to derive
// the R24 internal-money door list AT DEPLOY TIME — a gate deciding which doors
// to attack, from source it could not fully see. `accept="image/*"` in a JSX
// attribute opens a "comment" that a regex closes sixty lines later, so the
// door list was one blind spot away from being short, and A SHORT LIST OF DOORS
// TO ATTACK PASSES. That is the same failure the laws had, on the one caller
// that runs against a live environment.
//
// TYPES FOR THE TYPESCRIPT CALLERS live in `strip-comments.d.mts` beside this
// file — see its header for why a declaration file and not JSDoc alone.
//
// The tokeniser's own reasoning — why it is not two regexes, and the three
// separate times the regexes were wrong — is on `stripComments` below. Read it
 ...
 *
 * Ordinary TypeScript (`code`) is the only context a comment can open in; a
 * `template` is between backticks; a `jsxTag` is inside `<` … `>`, where the
 * attribute names, quoted values and `{}` holes live; `jsxText` is between an
 * element's tags — prose.
 *
 * @typedef {{ kind: "code", braces: number }
 *   | { kind: "template" }
 *   | { kind: "jsxTag" }
 *   | { kind: "jsxText" }} ScanContext
 */
 ...
 * @returns {string}
 */
function commentReplacement(eaten, options) {
 ...
 * The one dial.
 *
 * `keepLength` blanks each comment with the SAME NUMBER OF CHARACTERS instead
 * of removing it, so every index into the result still points at the same place
 * in the original. One caller needs this — web/test/hooks-order.test.ts walks
 * brace depth and reports `path:offset` — and it used to hand-roll the whole
 ...
 *
 * @typedef {{ keepLength?: boolean }} StripOptions
 */
 ...
 * @returns {number}
 */
function endOfTypeArguments(src, at) {
  let depth = 0
 ...
 * @returns {boolean}
 */
function opensJsxElement(src, at) {
