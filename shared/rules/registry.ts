...
export const NO_NESTED_SCROLL_EXEMPT: Record<string, string> = {
  ...
  // ── PENDING: her question 4 (tables and boards, Round 27) is still open ─
  "web/components/assistant/agent-history-tab.tsx#min-h-0 flex-1 overflow-y-auto":
    "pending Aurora's answer on tables and boards, 20 Sep 2026 — the assistant panel's own History tab list of past conversations. Decide item: is this the assistant pane's one sanctioned scroll (swapping in for the thread) or a second, nested one.",
  "web/components/assistant/agent-panel.tsx#max-h-[40vh] min-h-0 overflow-y-auto":
    "pending Aurora's answer on tables and boards, 20 Sep 2026 — the paused-turn confirm-steps list inside the assistant panel. Decide item: this can scroll alongside the thread's own scroll at the same time, which reads as two moving regions in one aside.",
  "web/components/tickets/tickets-dashboard.tsx#absolute inset-0 flex flex-col gap-2 overflow-y-auto":
    "pending Aurora's answer on tables and boards, 20 Sep 2026 — the tickets dashboard's own stacked-bar chart column. Decide item: a dashboard widget's internal scroll, not chat/rail/overlay.",
  "web/components/screens/import-screen.tsx#max-h-48 overflow-auto rounded-[var(--radius)] bg-surface-panel":
    "pending Aurora's answer on tables and boards, 20 Sep 2026 — the import screen's rejected-rows preview. Decide item: a bounded preview list, not chat/rail/overlay.",
  "web/components/meetings/meeting-detail.tsx#max-h-96 overflow-y-auto rounded-[var(--radius)] bg-surface-panel p-3":
    "pending Aurora's answer on tables and boards, 20 Sep 2026 — the meeting transcript preview. Decide item: a bounded preview panel, not chat/rail/overlay.",
}
