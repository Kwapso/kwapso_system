...
export const SECTION_HOSTED_ELSEWHERE: Record<string, string> = {
  ...
  // 2026 with clause (v). The three above are TAB sections somebody subtracted;
  // `processes` was never on the "This team" list at all — it is
  // `placement: "contextual"`, and the widened census found that no literal
...
export const RECORD_TABS_SINGLE_PANEL: Record<string, string> = {
   ...
   * (`ACTIVITY_GATE_MAP`).
   *
   * THE LIST IS EMPTY AND THAT IS THE GOAL, not an oversight: every record
   * detail in the app now draws a real tab strip, so R2's exemption list has
   * nothing left to excuse. It stays declared because `record-detail-tabs` reads
   * it, and because the next single-panel record needs somewhere to say so. */
}
