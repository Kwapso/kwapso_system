...
export const TOOLBAR_EXEMPT: Record<string, string> = {
  ...
  // ── THE LOGS DASHBOARD'S OWN ROW (R48 census ii, added 23 Sep 2026).
  "web/components/work/time-screen.tsx":
    "NOTHING ON THE TAB TO SIEVE. Aurora's ruling, 23 Sep 2026, gave the Logs module two tabs and named exactly what goes above the first one: \"add toolbar w filters by person, account\". The Dashboard tab draws six GROUPED PICTURES over a door read, not rows a browser is holding, so a search box there could only ever narrow nothing: the thing a person searching for a row of time wants is the ENTRIES tab one click away, whose `<PagedFind>` draws an unconditional `<SearchInput>` that asks the DOOR (R14 — 2,940 rows arrived from two years of the previous system). This is the same reasoning `tickets-dashboard.tsx` carries for drawing no toolbar at all after her 17 Sep ruling; the difference is that she asked for the two filters here by name, so the row exists and this line is about the one slot it leaves empty rather than about the row's absence. THE ROW IS STILL HELD TO R50: its `empty` is derived from the collection's own exact server count, never a literal.",
