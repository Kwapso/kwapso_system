...
export const TOOLBAR_EXEMPT: Record<string, string> = {
  ...
  "web/components/tickets-dashboard.tsx":
    "NOTHING ON THIS TAB IS TEXT A PERSON COULD SEARCH. R48's own stated ground is that a search box over a handful of rows is a control that cannot do anything; here the ground is one step further along — there are no rows, of any number. Every mark on the Tickets dashboard is a tally the database took over the whole backlog (open work per kind and stage, per system, per client, the recategorisation matrix, the closing-time quantiles and, in its own panel beside them since 6 Sep 2026, their twelve-month trend), and the only words on screen are the team's own ticket-type vocabulary and its client and system names, each of which is already a FILTER on this row rather than a haystack. A box here could only ever narrow a chart by a substring of a label, which is not a question anybody asks of a picture. THE SEARCH FOR TICKETS THEMSELVES IS ONE TAB AWAY and is the real one: `<PagedFind>` on the same screen, asking the door, over the paged collection (R14). Same file, same tab strip, so nothing became unreachable — it stopped being on the tab that has nothing to find. See TOOLBAR_SORT_EXEMPT's entry for the same component for why it has no sort either, and note the two are the same sentence twice: a dashboard is not a list. UNCHANGED BY THE APP-SCOPED VIEW (6 Sep 2026): the same component now also draws this row inside an app record's Tickets tab, narrowed to one system, where it carries a view switch back to the list. Fewer panels, the same argument — there are still no rows on it, of any number. AND UNCHANGED BY THE ROW GAINING ITS CREATE BUTTON the same day (\"on the dashboard, I'm missing the full toolbar\"): the row now carries the host's own \"Raise ticket\" node in `actions`, the identical control the list tab draws, which is the slot that was actually missing. A button that RAISES a ticket is not a box that FINDS one — R50 already makes the two disappear together on an empty collection, and R48's question here is unchanged, because raising a ticket from a chart is a sensible act and searching a chart is not.",
