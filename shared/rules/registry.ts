...
export const RULES_REGISTRY: Rule[] = [
  ...
  },
  {
    id: "R88",
    dimension: "ui",
    law: "EMPTY-STATE SINGLE DOOR: when a section is empty, its empty state's \"Add the first\" is the ONE way to add — never a second, header-drawn add button beside it — and the section's OWN HEADER (title row) draws nothing either, because the empty state's own heading stands in for it. The client's ruling, 18 Sep 2026, reading the deployed Work logs card back over her own earlier one (R50/R84): \"we already said on empty state, we only have the first, not the top-right plus button. This is a law. Reinforce it everywhere. And then also remove the work log header when it's empty.\" Her screenshot: the ticket page's Work logs card, empty, drew a \"Work logs\" TITLE with a black top-right `+` (`AddButton`) AND, in the body, `CollectionEmptyState`'s own \"No time logged against this yet. [...] Add the first\" — two doors on one zero-row collection. R50/R84 already close the BUTTON half everywhere a title row sits inside a `<ToolbarRow>`; the gap R88 closes is the title row that sits OUTSIDE one — `AddButton`'s own `empty` prop is optional for exactly that shape, and two call sites (`help-detail.tsx`'s Related stories and Work logs panels) had used the escape hatch to argue the opposite: a hardcoded `empty={false}`, reasoned in `EMPTY_TOOLBAR_EXEMPT` as \"this button should stay reachable at zero rows.\" That reasoning is retired. `EmptyGatedPanel` (`web/components/deep-link/screen-bits.tsx`) is the one shared shell this law lives in: `empty` true drops the WHOLE header — title, count and action together — and draws only `children`, expected to be the panel's own `CollectionEmptyState`; `children`'s position in the returned tree never moves as `empty` flips, so a child that owns its own \"add\" dialog is never remounted by the header appearing or disappearing. CHECKED as a source census, `web/test/empty-state-single-door.test.ts`: (i) `EmptyGatedPanel`'s own source must gate its header block behind `{!empty && (` — the central guard every call site leans on, the same shape R50 holds `ToolbarRow`/`AddButton` to; (ii) every `<AddButton>` tag outside a `<ToolbarRow>`'s `actions`/`renderActions` slot (screen-bits.tsx itself excepted, which declares both) must sit inside an `<EmptyGatedPanel>` tag's own span, or be named in a reasoned `EMPTY_STATE_SINGLE_DOOR_EXEMPT` entry — rot-checked both ways, so the list can only shrink. It is empty as of 18 Sep 2026: both offending sites were fixed by adopting the shell rather than by writing an exception for the shape the ruling retired.",
    checkId: "empty-state-single-door",
    status: "enforced",
  },
  {
    id: "R89",
    dimension: "ui",
    law: "A RECORD SCREEN'S FOOTER SITS AT THE SCREEN'S OWN BOTTOM EDGE, WITH THE PANEL GAP ABOVE IT, NOT MERELY LAST INSIDE ITS OWN CARD. The client's ruling, 18 Sep 2026, verbatim, over the shipped ticket page: \"On ticket detail, the footer should be at the very bottom. The position is still fucking wrong. Fix it once and for all.\" D21 (`footer-is-last`, `web/test/footer-is-last.test.ts`) already proved a `<CardFooter>`/`<ReplyComposer>` is the LAST rendered child of its own enclosing card — DOM order only, nothing about where that card sits ON THE SCREEN. It was not enough: measured live on staging (T0001, headless Playwright, before this law's fix) the conversation card's own footer closed at y=1298 against the screen body's own visible bottom at y=884 (1440×900) — a footer correctly last inside a card that was itself floating 414px past the window, because `TicketDetailBody`'s conversation card was sized to the SUM of the three side panels' own content height (an EARLIER, different ruling, the client's own \"the addition of the three of the right, same as conversation\"), never to the screen's actual available height. \"AT THE VERY BOTTOM\" NAMES THE SCREEN, and this law is built BY CONSTRUCTION, the same standard every structural law in this base holds to — no magic pixel offset, no viewport-relative guess. `app-shell.tsx`'s own R29 page container (the one div it wraps `{children}` in, besides `<LiveStatus/>`) was ALREADY a `flex-col` box floored at the screen body's own visible height (`min-h-full`) — a `min-height` flex container hands any leftover space (floor minus its items' own content height) to whichever item carries `flex-grow`, so no edit to that file was needed. `TicketDetailBody`'s own root became that flex-grow item (`lg:flex-1 lg:min-h-0`, marked `data-slot=\"ticket-detail-body\"`), its grid collapsed from three separate auto-placed right-column rows to ONE cell (a `flex flex-col gap-6 overflow-y-auto` wrapper around the same three panels) because CSS Grid's default stretch splits leftover space across every `auto` track rather than filling a single one, and the conversation cell's own `lg:h-full lg:min-h-0` now resolves against a REAL, definite remaining height rather than the right column's own content sum — `CardFooter` sitting flush at that cell's bottom edge, at every viewport height, with the side column scrolling independently the moment its three panels are taller than the row. THE COMPOSER'S OWN HALF — Aurora's screenshot 4, verbatim: \"This composer should have a background color that makes it easy to identify, and also it should be full width of its own container.\" Measured live before the fix: the composer's own background and the conversation card's own ground were the identical `rgb(247,242,235)` (`--surface-panel`) — no contrast at all, because an earlier comment's own reasoning (from before R67 repainted the conversation card's ground to `--surface-panel`) had gone stale without anyone revisiting it — and the composer's own rendered width was 271px inside a 769px footer. Fixed to `w-full` plus `bg-card`, the SAME class the kit's own `Input` (`shared/ui/components/input/input.tsx`) paints every ordinary text field with — matching every input in the app by construction, not by a new colour chosen to look right once. AMENDED, 18 Sep 2026 EVENING — THE FIRST LANDING DID NOT HOLD, RE-PROVEN LIVE AGAINST A REAL TICKET: this paragraph's own claim that `min-h-full` \"hands any leftover space ... so no edit to that file was needed\" is false the instant real content is taller than the floor — `min-height` is only ever a floor a `height:auto` block GROWS PAST, never a cap, so once content exceeds it there is no leftover space for any `flex-grow` descendant to consume. Re-measured (`${SCRATCH}/footer-fix-dump.json`) against T0001 with a real conversation: the page container stood 1312px against a 785px pane, and the conversation card still closed 526px past the screen body. THREE fixes, found by injecting candidate CSS live and re-measuring before touching source (`${SCRATCH}/footer-fix-proof.json`): (1) `app-shell.tsx`'s page container is `h-full` now, not `min-h-full` — a DEFINITE height, so the `flex-1 min-h-0` chain below it has a real budget instead of infinite room to grow into; safe for every other screen because nothing here sets `overflow`, so taller content still paints past the (now fixed) box and `screen-shell-body` still scrolls all of it — proved pixel-identical, before/after, on the tickets dashboard and an account detail page. (2) With that column finally definite, `RecordChrome` (the ticket's own head, a SIBLING of `TicketDetailBody` because `panelVisible={false}`) was ALSO carrying `flex-1 min-h-0`, splitting the column's real height 50/50 with the grid instead of taking only its own content height — fixed with `record-chrome.tsx`'s own `HEAD_ONLY` (`flex-none`), read off `panelVisible` directly. (3) The conversation card's `min-h-[420px]` (a below-`lg` floor for the stacked case) outlived the breakpoint it was written for and pinned the card 48px past the grid's own real row — released with `lg:min-h-0`, the pattern `tickets-dashboard.tsx` already uses for its own stacked floors. Re-proven live at 1440×900, 1280×800 and 1024×768: the card's footer sits flush with the screen body's own bottom inset at every width, the side column scrolls independently, and the composer's PARENT div (the form itself always carried `w-full`; the bug was that its wrapper — the sole child `<CardFooter>`'s flex ROW renders — had no width claim of its own, so the form's `w-full` was 100% of an already shrink-to-fit box) now carries `w-full` too. AMENDED AGAIN, 19 SEP 2026 — THE LG FIX ABOVE NEVER TOUCHED BELOW `lg`. Aurora, verbatim, over the same page: \"No, this is still wrong. The footer is currently under the stages and above the content. … I cannot believe you're so stupid and you cannot fix this.\" Below `lg` the old grid collapsed to one column and stacked the conversation card (thread AND composer together) FIRST, the three side panels SECOND — exactly her sentence, confirmed live at 760×900 (rail collapsed) before this fix: the conversation card closed 434px past the visible screen body, the side panels starting AFTER it. Settled the same session, so a future reader is not sent re-discovering it: `lg:` here is a plain viewport media query with no `container-type` upstream of it, so opening the assistant panel (which narrows the CONTENT column, not the viewport) never crosses it — proven live at 1440×900 with the assistant open and at 1024×768, both already-passing LG states, untouched by this amendment. THE LAW NOW READS: AT EVERY WIDTH THE COMPOSER IS THE LAST THING ON THE SCREEN AND SITS ON ITS BOTTOM EDGE. A first, CSS-only attempt (one tree, `order-*` walking the two columns back to their LG positions below `lg`) broke on the first live proof against a real ticket: the head alone measured ~74% of the available budget at 760×900, and two SEPARATE `flex-1 min-h-0` regions (the side panels and the conversation) crushed the conversation to a real, live, on-screen height of zero — a flex item with `flex-basis:0` and `min-height:0` shrinks toward its basis, not its content, the instant there is no surplus to grow into. Fixed instead with a real breakpoint DECISION, not a resized CSS tree: `TicketDetailBody` (`ticket-detail-body.tsx`) now picks its own tree with `useIsAtLeastLg()`, a `useSyncExternalStore`/`matchMedia` hook in the same house shape `web/lib/use-is-phone.ts` already banks for \"a phone needs a different tree, not a resized one\" — because the composer is ONE stateful control (a DOM `field` ref, a held-reply countdown, an attach-tile grid) and cannot ALSO stay mounted a second time behind a CSS toggle without becoming two instances racing for one ref. At `lg` the tree is byte-identical to the first landing above — same classes, no `lg:` prefix needed since JS already gated the branch. Below `lg` it is `flex flex-col`, filling the SAME `flex-1 min-h-0` budget the LG grid already claimed against `app-shell.tsx`'s own page column (unchanged, needed no edit); ONE scrolling region (never two competing ones) holds the side panels FIRST and the conversation's bare THREAD (no footer inside it) SECOND — a plain `Card`/`CardContent`, nothing rendered after it competing for the composer's own space; and the composer — the kit's own `<CardFooter>`, the IDENTICAL element `TicketConversationPanel` uses at `lg`, never duplicated — is this flex column's own `flex-none` LAST child, carrying its own `bg-surface-panel` fill since it no longer stands inside a `Card`. `TicketConversationPanel`'s own Card lost its below-`lg` floor (`min-h-[420px]`) and its `lg:` prefixes entirely — dead weight once it only ever renders inside the LG branch. AMENDED A FOURTH TIME, 19 SEP 2026 (ROUND 22B) — \"BOTTOM EDGE\" NAMES THE CARD, NOT A GAP NEAR IT. Aurora, over a screenshot at 1784×981 (rail collapsed): \"look at screenshot! that's the footer not being on the very bottom! fix this at once.\" Measured live before this fix (`${SCRATCH}/r22-footer-dump-out.json`): the conversation card's own `CardFooter` closed at y=941 while `[data-slot=\"screen-shell-body\"]` — the real scroller, the one box a footer's bottom edge actually answers to — closed at y=965, a further 24px of nothing under an already-correct 32px card inset (`CardFooter`'s own `lg:pb-[var(--space-7)]`). Every earlier landing measured the wrong reference: \"the screen body's own bottom\" was read as wherever the flex chain's own `h-full` box happened to stop, never against the vendored pane it sits inside. `h-full` floors that div to `screen-shell-body`'s CONTENT-box height — real and definite, exactly as the first AMENDED paragraph fixed it to be — but a content box, by definition, excludes the pane's own `padding-bottom` (`DENSITY_BODY`, screen-shell.tsx, `py-[var(--space-5)] lg:py-[var(--space-6)]`, this shell's default density): 20/24px reserved as blank margin below every screen's content, correct everywhere else, and exactly the \"page inset\" this law now forbids under a footer that is supposed to be flush. THE LAW NOW READS, PRECISELY: at every width, the conversation card's own bottom edge coincides with the screen body's own bottom edge — no page inset under it — and the composer sits at the card's own inner inset from that edge, nothing more. `shared/ui/` cannot be hand-edited to drop that padding for one screen (R39), so `app-shell.tsx`'s own page container grows INTO it instead, and only where a ticket body is actually present: `has-[[data-slot=ticket-detail-body]]:h-[calc(100%+var(--space-5))]` / `lg:has-[[data-slot=ticket-detail-body]]:h-[calc(100%+var(--space-6))]` — the pane's real, full border-box height, to the pixel, proved live before touching source (`${SCRATCH}/r22-footer-inject-out.json`): container bottom 965.00 against `screen-shell-body` bottom 965.00 at 1784×981, 884.00/884.00 at 1440×900, `/accounts` and the tickets dashboard pixel-identical with and without the rule (no `ticket-detail-body` marker on either, so `:has()` never matches). The below-`lg` pinned `<CardFooter>` needed no separate fix: `data-slot=\"ticket-detail-body\"` marks BOTH of `TicketDetailBody`'s own trees, so the same rule reaches it and its own `p-4` inset is now the only thing between it and the pane's real bottom edge. AMENDED A FIFTH TIME, ROUND 23, 19 SEP 2026 — Aurora, over a screenshot at 1991×842 with the assistant panel OPEN (a width/height/assistant-state combination none of the four earlier amendments' own proof widths had ever measured together): \"THE PROBLEM IS WHERE THE FOOTER IS!!! SHOULD BE AT THE VERY BOTTOM!\" THE LAW NOW READS, FINAL FORM: THE COMPOSER IS PINNED AT THE BOTTOM OF THE SCREEN AT EVERY WIDTH AND HEIGHT; EVERYTHING ELSE SCROLLS ABOVE IT. Every earlier amendment bounded the CONVERSATION CARD's own height and trusted a long `flex-1 min-h-0` chain (six-plus links, through a vendored shell this app cannot edit, R39) to resolve correctly on every future width/height/assistant-state combination — which is precisely the shape that kept re-breaking on the next untested combination. Round 23 stops chasing the chain and instead makes the composer's own visibility true BY CONSTRUCTION, twice over: `TicketDetailBody`'s two per-width trees (`useIsAtLeastLg()`, unchanged) now differ ONLY in the DOM order of the scrolling region above the composer (the grid-vs-stack choice, for the side-panels-before-thread reading order below `lg`) — the composer itself is pulled OUTSIDE that scrolling region entirely, ONE `<CardFooter>` instance in ONE DOM position, `flex-none`, common to both trees, so no per-width branch can strand it in two different homes again. `TicketConversationPanel` — the component that used to bundle the thread's `CardContent` and the composer's `CardFooter` inside one `Card` at `lg` — is retired; the thread's own footer-less card is now built identically in both trees. And, belt-and-braces: the composer's own `<CardFooter>` carries `position: sticky` in addition to its place in normal flow, so if the flex chain above it ever miscalculates again on some new combination, the composer still cannot leave the viewport — sticky answers to the nearest SCROLLING ancestor (`[data-slot=\"screen-shell-body\"]`, the vendored pane), not to this file's own flex arithmetic. THE STICKY OFFSET IS NOT `bottom-0`, MEASURED LIVE: a plain `bottom-0` anchors to that pane's own PADDING edge (`DENSITY_BODY`'s `py-[var(--space-5)] lg:py-[var(--space-6)]`), and clamped the footer a live-measured 24px short of the pane's true bottom at `lg` — sticky was undoing exactly the padding the fourth amendment's own `has-[[data-slot=ticket-detail-body]]:h-[calc(...)]` growth rule exists to let the flow position grow past, the two mechanisms fighting each other. `bottom-[calc(-1*var(--space-5))] lg:bottom-[calc(-1*var(--space-6))]` — the SAME two tokens the growth rule itself reads — cancels that padding so sticky's own \"stuck\" threshold coincides with the pane's real border-box bottom instead of stopping short of it, proved live both ways (24px with `bottom-0`, 0px with the negative offset) at 1991×842, 1440×842 and 1784×981 with the assistant open, including with 40 injected thread messages and 18 injected side-panel rows (a live stress proof of \"no matter how tall the head, the activity card or the thread are\"). AMENDED A SIXTH TIME, ROUND 24, 19 SEP 2026 — THE LAW HAD PINNED THE WRONG ELEMENT. Aurora, over her own screenshot, verbatim: \"the black section, the footer, should be at the very bottom / why is the write text space full width?? rewind here / THE FUKING FOOTERRR!\" Every round from the first landing through round 23 read \"the footer\" as the reply composer, and round 23's whole construction pinned the composer at the screen's bottom, sticky, outside every card. She meant THE BLACK BAND: the kit's own CH27.8 ink footer, the dark card holding LATEST ACTIVITY (the recent rows + \"Add a note\") and RECORD (Created by / Last edited by) — which had been rendering correctly last inside ITS OWN small DOM region the whole time, but in the WRONG PLACE ENTIRELY: right after the stage ladder and before the ticket's own scrolling body, because `RecordScreen`'s one `<RecordDetail>` call draws band/hero/panel/footer in one fixed order and the ticket's body is a SIBLING rendered after that call — no prop on it could ever move the footer past a sibling. THE LAW NOW READS, FINAL FORM: THE TICKET'S DARK BAND (LATEST ACTIVITY + RECORD) IS THE PAGE'S FOOTER — THE LAST ELEMENT, PINNED AT THE BOTTOM OF THE SCREEN AT EVERY WIDTH AND HEIGHT; THE CONVERSATION AND SIDE CARDS SCROLL ABOVE IT; THE REPLY COMPOSER SITS AT THE CONVERSATION CARD'S FOOT, INSIDE THE CARD, AT EVERY WIDTH. Fixed by a SECOND call to the kit's own composition rather than a moved prop — DOM order between two sibling elements is decided by where each is CALLED, not by a prop on either: `record-chrome.tsx` exports `RecordFooterBand`, calling the kit's `RecordDetail` a second time with nothing but the footer's own props (no title, no hero, no tabs, `panelVisible={false}`), so `hasBand`/`hasHero`/`visibleTabs`/`panelBody` all read false or empty and the ONLY region left standing is the footer card — the kit's own composition and tokens, reused rather than redrawn (R39). `RecordScreen`'s own copy is switched off (`footerVisible={false}`, a new passthrough prop) so the same card never draws twice. `TicketDetailBody` (`ticket-detail-body.tsx`) renders `RecordFooterBand` as its own `flex-none` LAST child, cropped by an `overflow-hidden rounded-t-[var(--radius)]` wrapper (R31's own named exception for a surface that meets the screen's true bottom edge, since the kit's footer `Card` carries no prop to narrow its own four-corner radius to one edge). `TicketConversationPanel` — retired in round 23 — is UN-RETIRED, exactly its round-22 shape (`git show d1167183`): `Card` → `CardContent` (the thread, scrolling) → `CardFooter` (the composer, `Card`'s own last child, D21), at every width now, because the BAND is what now guarantees \"always visible at the bottom,\" not the composer needing a second, below-lg home outside any card. `w-full` on the composer is therefore 100% of the CARD's own inner width, never the page's — answering her \"why is the write text space full width??\" directly: the bug was the composer's `w-full` having become 100% of the page's own pinned band under round 23, not the card it used to sit inside. PROVED BY CONSTRUCTION, NOT `sticky` ALONE — live injection this round (40 thread bubbles + 6 activity rows, 1991×842 and 1440×842 with the assistant open, against T0001, DOM-moved and class-patched live before touching source) found a sticky band riding on top of an UNBOUNDED scrolling region rides DOWN with it the instant that region's own content outgrows the viewport: `ticketBody.clientHeight` measured 7px and the thread's own `[role=\"log\"]` was never actually capping (`scrollHeight === clientHeight`, growing rather than scrolling) before this round's fix. Fixed by binding every link in the chain FIRST — `h-full min-h-0` (at `lg`, a grid cell already at 100% of its row) or `flex-1 min-h-0` (below `lg`, a stack item sharing the column with the side panels' own natural height) on the conversation cell, `h-full min-h-0` on `TicketConversationPanel`'s own `Card`, and `min-h-0 overflow-y-auto` added to its `CardContent` (the ONE true scroller) — so the scrolling region is bounded BY CONSTRUCTION and the band's own position falls out of that bound; `position: sticky`, carrying round 23's own negative, padding-compensated `bottom` offset, stays on the band as belt-and-braces only, never the primary mechanism. Proved live (`${SCRATCH}/band-proof.json`, `${SCRATCH}/band-*.png`) at 1800×978 (rail expanded), 1991×842 (assistant open), 1440×900, 1280×800 and 760×900 (rail collapsed), plus the 40-bubble/6-row stress at 1991×842 and 1440×842: the band's bottom sits flush with the pane's own bottom edge (0px) at every width `md:` (768px) and above, the thread scrolls inside the card (not the page) at every width including under stress, and the composer's own rendered width equals the card's inner width (the card's own width minus its `CardFooter` `p-4` inset, 32px, never the page's). BELOW `md` (760×900), the app's own PRE-EXISTING, unrelated `pb-24 md:pb-0` mobile bottom-chrome reservation on `app-shell.tsx`'s page container (measured IDENTICAL on the untouched round-23 composer before this round's own change) leaves a 96px gap between the band and the pane's raw border box; the band still sits flush with the page's own real CONTENT edge, and this gap is not a regression this round introduced. THE ONE HONEST LIMIT FOUND: under the 6-injected-activity-row stress at 1440×842 the band's own natural content height (606px) exceeded the whole page's available budget (532px) and the excess made `[data-slot=\"screen-shell-body\"]` itself scroll rather than the band clipping — a scenario the real component can never produce, since `footerActivityItems` always slices to the newest THREE rows server-side; logged here rather than silently declared impossible. AMENDED A SEVENTH TIME, ROUND 26, 19 Sep 2026 — THE PANEL GAP ABOVE THE BAND. Aurora, reading the round-24 page back, verbatim: \"ok, now the footer is at the bottom, but there's a law about how much margin there must be above!!! add the space!!\" The band's own flush bottom edge (round 24) was already correct and stays exactly 0px; what was missing was the ordinary space ABOVE it — the same `gap-6` (24px at the 16px root, `documents/UI-RULEBOOK.md`'s own S1/N7, \"between panels on a screen: gap-6\") every other pair of stacked panels on this screen already carries. `TicketDetailBody`'s own root (`data-slot=\"ticket-detail-body\"`) — a flex column whose only two children are the scrolling region and the band — now carries `gap-6` itself, so the gap sits in NORMAL FLOW between the two children, by TOKEN rather than a hand-numbered offset, and never touches the band's own `sticky` flush-bottom mechanics. THE LAW NOW ALSO READS: the band's own top edge carries the standard panel gap (`gap-6`, 24px) above it, exactly the gap between any other two stacked panels on the screen, while its bottom edge stays flush at 0px. AMENDED AN EIGHTH TIME, ROUND 27, 19 Sep 2026 — THE SIDE COLUMN NEVER SCROLLS. Aurora, over the live ticket page, verbatim: \"there should be no scrolling to see all right column items — expand the height!\" and \"scroll only on conversation when taller than right column.\" Round 24's own `lg` grid (`h-full min-h-0` on the grid AND the side column, `overflow-y-auto` on the side column) forced the ROW to fill the whole scrolling region regardless of content, so a short ticket showed the side column scrolling inside its own little box while the conversation card sat mostly empty beside it — backwards: the side column's three cards are what a person needs to see in FULL every time, the conversation is what varies wildly (a two-line ticket vs. a forty-message thread). Rebuilt by construction, not by swapping which side gets the scrollbar: the grid and the side column both drop their own `h-full`/`min-h-0` (and, on the side column, `overflow-y-auto`) — content-sized now, like any ordinary block, so the row resolves to the side column's own natural height (an explicit-height grid with one `auto` row would otherwise let `align-content`'s own default `stretch` hand that row the FULL container height regardless of content, exactly what forced the old internal scrollbar); the conversation CELL drops `h-full` for `relative min-h-0` — no height class of its own at all, sized only by the grid's own `items-stretch`, to whatever the row resolved to; and `TicketConversationPanel`'s own `Card` takes a new `fill=\"absolute\"` at `lg` (`position: absolute; inset: 0`) — taken OUT of normal flow so the cell contributes ZERO intrinsic height back to the grid's own auto-track sizing, the one piece that keeps a long thread from dragging the row (and the side column with it) taller. The thread still scrolls, inside the now-absolutely-positioned card, via its own `CardContent`'s `min-h-0 flex-1 overflow-y-auto`, unchanged — only now against a height the SIDE COLUMN set, never the other way round. PROVED FIRST by DOM/style injection against the live page, mirroring this exact construction (`${SCRATCH}/row-proof.json`, `${SCRATCH}/row-*.png`), at 1800×978, 1991×842 assistant open, 1440×900 and 1280×800: the side column's own `scrollHeight === clientHeight` (never scrolls) and its own bottom exactly matching its last card's bottom (0px) at all four; the conversation card's height matching the side column's own height exactly (0px diff) at all four; a 40-bubble stress injection at 1991×842 leaving the row height UNCHANGED (810.89px before and after) with the thread still scrolling inside the card; three injected extra side cards at 1440×900 (side column grown to 1620.38px, taller than the 900px screen) making the ONE scrolling region above the band scroll the whole page (`regionScrolls: true`) while the band stayed flush (0px vs. the screen body) with its own round-26 panel gap unchanged (24px, measured after scrolling the region to its own end) above it. THE LAW NOW ALSO READS: AT `lg` THE ROW IS AS TALL AS THE SIDE COLUMN, WHICH NEVER SCROLLS; THE CONVERSATION MATCHES IT AND SCROLLS INSIDE ONLY WHEN TALLER. AMENDED A NINTH TIME, ROUND 28, 19-20 Sep 2026 — ONE PAGE SCROLL, NO INNER SCROLLBAR. Aurora's dark-theme screenshot showed a SECOND scrollbar inside the ticket page, above the band: this file's own \"ONE SCROLLING REGION\" wrapper (`flex-1 min-h-0 overflow-y-auto`, rounds 24-27) was a real `overflow:auto` box bounded to whatever the flex chain above it resolved to, so once the grid/stack inside it needed more room than that the box scrolled ITS OWN content instead of letting the page grow — every earlier round protected the BAND's position from exactly this shape and never noticed it had built one at the REGION's. Her ruling, restated as the standing R91 (`no-nested-scroll`): \"never need to scroll to see al content!!! (only exception chat compnents)\" — one page scroll only, and R91's own exception list names this file's chat thread `CardContent` by name. FIXED BY REMOVING THE BOUND, NOT MOVING IT: (1) `TicketDetailBody`'s own root drops `min-h-0`, keeping `flex-1` — flexbox's own \"automatic minimum size\" (the initial value of `min-height` is `auto`, a content-based floor for a flex item along the main axis whenever nothing overrides it to `0`) fills the leftover space after the head when content is short and grows PAST that leftover space to the root's own content height when content is taller, with no percentage arithmetic and no fight with the head sibling. (A literal `min-h-full`, the first candidate tried by live injection, FAILED the short-page proof by ~196px, exactly the head's own height — `min-height:100%` on a flex item resolves against the WHOLE flex container, blind to the head sibling, so the root was floored to the container's full height ON TOP OF the head.) (2) the scrolling-region wrapper drops `flex-1`/`min-h-0`/`overflow-y-auto` entirely, an ordinary content-sized block like the side column already was (round 27). (3) the band drops `sticky` and the round-23 negative, padding-compensated `bottom` offset for `margin-top:auto` — the ordinary \"footer at the bottom of a short page\" flex trick, consuming whatever leftover space the root was handed. (4) `app-shell.tsx`'s own `has-[[data-slot=ticket-detail-body]]:h-[calc(100%+var(--space-5))]` / `lg:has-[...]:h-[calc(100%+var(--space-6))]` growth rule (round 22b) is RETIRED — it existed only to let the STICKY band reach past the pane's own reserved bottom padding, and nothing here is sticky any more, so the page container keeps its ordinary padding and the ticket page ends on it exactly like every other record screen. PROVED FIRST by live DOM/style injection against T0001 on staging, twice (`${SCRATCH}/onescroll-proof.json`, the failed `min-h-full` candidate; `${SCRATCH}/onescroll-proof2.json`, this construction), before touching source, at 1800×978, 1991×842 assistant open, 1440×900, 1280×800 and 760×900: after injection, the census of every element inside `[data-slot=\"ticket-detail-body\"]` whose `scrollHeight` exceeds its `clientHeight` came back EMPTY at all five widths (before injection it always found exactly the old region wrapper, diffs of 360-1270px depending on width); `screenBodyScrolls` was `true` (the one page scroller) and `docScrolls`/`threadScrolls` `false` at rest, at every width; the band stayed the root's last child, flush with the screen body's own bottom (within ±0.6px, ordinary sub-pixel rounding); the conversation card matched the side column's own height exactly at `lg` (A/B/D/E — F, 760×900, is below `lg`, stacked rather than paired, unchanged from round 27). At 1991×842 with the assistant open, 40 injected thread bubbles left the root's own height UNCHANGED (1057.19px before and after) with the ONE remaining scroller being the thread's own `card-content` (R91's named exception, nothing else). At 1440×900, removing the three side cards and blanking the thread (a genuinely short ticket) landed the band's own bottom EXACTLY at the screen body's own bottom (0px — the `min-h-full` candidate had missed this by ~196px). And against an account record's own detail page (untouched by this construction), the last card's own bottom sat within ~1px of the screen body's own bottom at every one of the five widths — proving the ticket page's band now ends exactly where every other record screen's last card already does.",
    why: "Earned the same way D21 was: a law that reads DOM order alone cannot see where the ordered card itself sits, and \"last child of a card\" and \"at the screen's own bottom\" are different sentences that happened to agree on the one page D21 was written against, until a later ruling on the same page's own right column changed the card's height and pulled the two sentences apart again — and then a SECOND time: the FIRST construction also read as correct against a green build, because every check it wrote asked whether a class was PRESENT, never whether the construction it sat inside actually resolved to a real number once real content, not an empty fixture, sat inside it. And a THIRD time, 19 Sep 2026: the LG fix's own proof widths (1440×900, 1280×800, 1024×768) were all AT or ABOVE the `lg` breakpoint, so a law that reads \"at every viewport height\" was never actually read against a viewport BELOW it — the exact same shape of gap, one axis over. And a FOURTH time, the same day: every one of those proof widths measured \"the flex chain's own box\" as the screen's bottom, never the vendored pane the chain sits inside — so a real, uncounted 20/24px of that pane's own reserved `padding-bottom` (`DENSITY_BODY`, screen-shell.tsx) sat under an already-correct card inset, invisible to a check that asked only whether the CLASSES were present, never what pixel they resolved to against the pane's own true edge. AND A FIFTH TIME, round 23: every one of those four amendments' own proof widths tested the assistant CLOSED, or open at a ~900px-tall viewport — never open AND short — so a law that reads \"at every width and height\" still had an entire axis (assistant-state × short height) nobody had proved. `web/test/footer-on-the-edge.test.ts` now reads `app-shell.tsx`'s own page container for `flex-col` + a DEFINITE `h-full` PLUS the `has-[[data-slot=ticket-detail-body]]:h-[calc(100%+var(--space-5))]` / `lg:has-[...]:h-[calc(100%+var(--space-6))]` pair, `record-chrome.tsx` for its own `HEAD_ONLY` constant and the `panelVisible ? FOOTER_TO_BOTTOM : HEAD_ONLY` wiring, and `ticket-detail-body.tsx` for its own `useIsAtLeastLg()` breakpoint hook, its ONE scrolling region per tree (side panels then thread below `lg`; the 2fr/1fr grid at `lg`), and the composer's own `<CardFooter>` — the ROOT's own LAST child in both trees, `flex-none`, `w-full`, `sticky` with its negative, padding-compensated `bottom` offset (never a plain `bottom-0`, which measured a live 24px gap), `bg-surface-panel` — asserted identically regardless of branch. BOTH the reply composer's own pill AND its wrapping div keep their `w-full`, plus the SAME background class read straight off `input.tsx`'s own `inputVariants` — never hand-typed, so a future kit change re-proves parity rather than silently drifting from it. AMENDED A SEVENTH TIME, ROUND 26, 19 Sep 2026 — the panel gap above the band. Aurora, reading the round-24 page back, verbatim: \"ok, now the footer is at the bottom, but there's a law about how much margin there must be above!!! add the space!!\" The band's own flush bottom edge (round 24) was already correct and stays exactly 0px; what was missing was the ordinary space ABOVE it — the same `gap-6` (24px at the 16px root, `documents/UI-RULEBOOK.md`'s own S1/N7, \"between panels on a screen: gap-6\") every other pair of stacked panels on this screen already carries. `TicketDetailBody`'s own root (`data-slot=\"ticket-detail-body\"`) — a flex column whose only two children are the scrolling region and the band — now carries `gap-6` itself, so the gap sits in NORMAL FLOW between the two children, by TOKEN rather than a hand-numbered offset, and never touches the band's own `sticky` flush-bottom mechanics. THE LAW NOW ALSO READS: the band's own top edge carries the standard panel gap (`gap-6`, 24px) above it, exactly the gap between any other two stacked panels on the screen, while its bottom edge stays flush at 0px. AND AN EIGHTH TIME, ROUND 27: a construction can be internally consistent — bounded, definite-height, proved live — and still answer the WRONG QUESTION, because nobody had asked which side of the row was allowed to decide its own height. Round 24 bounded both cells to the SAME row height without ever asking which one should SET it, so the natural reading (the side column, being fixed content, sets it; the conversation, being variable content, follows) was inverted by default the day the grid was first built, and every proof since measured only that both cells still agreed with each other — never that they had agreed on the wrong side. AND A NINTH TIME, ROUND 28: every one of rounds 24-27's own checks asked whether a class was PRESENT (`overflow-y-auto`, `flex-1`, `min-h-0` on the region; `sticky` on the band) rather than whether that construction ever created a SECOND scroller — so a check built to prove the band could never leave the viewport was blind to the region growing its own scrollbar one level up, the exact inversion of the bug R91 was written to forbid. `web/test/footer-on-the-edge.test.ts` now reads `ticket-detail-body.tsx`'s own root for `flex-1` WITHOUT `min-h-0`, the scrolling-region wrapper for the ABSENCE of `flex-1`/`min-h-0`/`overflow-y-auto`, and the band for `mt-auto` WITHOUT `sticky` or either bottom offset — and `app-shell.tsx` for the ABSENCE of the retired `has-[[data-slot=ticket-detail-body]]` growth rule, so a reintroduction of any of the four is caught the same session it lands, not the next time somebody happens to open the page in dark mode.",
    checkId: "footer-on-the-edge",
    status: "enforced",
  },
  {
    id: "R90",
    dimension: "ui",
    law: "FACES IN CHOICES: ANY CHOICE OVER PEOPLE, CONTACTS, ACCOUNTS OR APPS SHOWS THE SAME FACE THE LISTS SHOW. Aurora, verbatim, about the new Raised-by `Select` on the ticket form and page: \"every time there is an avatar, I want to also see it in the choice component, so I also want to see the avatars here.\" Law of the house rather than a one-off fix: the photograph where a record has one, initials on the record's own tone where it does not, in the OPTIONS and in the TRIGGER's own chosen value. THE KIT GREW THE SLOT FIRST (kwapso-design v1.2.127): `SelectItem` already carried an `image` prop, but it rendered a bare `<img>` with no fallback path — the exact silhouette mismatch RULES.md §4.4 already named for a picker's mark disagreeing with the record's own — and nothing at all drew a mark in the TRIGGER's own chosen value, because Radix clones `ItemText`'s children into the trigger and an `<img>` cloned there would have been a second, unasked-for drawing in the 44 field. `face={{ src, name, tone, shape }}` on both `SelectItem` and `SelectTrigger`, rendered through one shared `SelectFaceMark` helper over the kit's real `Avatar`/`AvatarImage`/`AvatarFallback` primitive, closes both gaps; the trigger's face is handed in by the CALL SITE (it already knows which option is selected — it is what supplies `SelectValue`'s own placeholder), keyed by the face's own identity so a stale `Avatar` load-status left over from a PRIOR selection's photograph can never survive into a new no-photo one (measured live in the kit's own `verify/select-faces`, this exact bug, before the fix). Wired in this app: `help-form-dialog.tsx`'s and `help-stakeholders.tsx`'s \"Raised by\" contact Selects (both options and trigger), and `work-logs-panel.tsx`'s \"Logged by\" staff filter (options and trigger; INITIALS only — the door behind it, `WorkLogSummary.people`, carries no `imageUrl`, a door change out of scope for this pass and left as an open, named gap rather than silently worked around). A STATIC CENSUS, `faces-in-choices` (`web/test/faces-in-choices.test.ts`), holds every `<Select>` to account: detected as a choice over a record by its own OPTIONS ARRAY'S FIELD NAMES (`personName`/`contactId`/`memberId`/`accountId`/`avatar`/`photo`/`initials`, read off the block itself or, through the same indirection `staff-pill-row.test.ts`'s own `helperFindings` sees through, off a pre-built options array's `const` definition elsewhere in the file), every `<SelectItem>` inside an identity-bearing block must carry `face=`. `FACES_IN_CHOICES_EXEMPT` is the reasoned, rot-checked way out, keyed by the source array's own EXPRESSION rather than a line number (a file:line key rots on every edit above it) — empty on the day this law shipped.",
    why: "SCOPED TO `<Select>` ON PURPOSE, AND NOT R35 WIDENED. R35 (`records-carry-their-face`) already holds `RecordPicker`/`PickerOption` to exactly this account, structurally, through the TYPE — `picture`/`mark`/`face` are fields `PickerOption` must declare, so a caller cannot drop one before a component ever sees it — which is a completely different, non-overlapping mechanism from a JSX prop on a newly-shipped kit component. Folding `RecordPicker` into this census would re-litigate ground R35 already owns and risk the two drifting against each other over the same population. The kit's `Select` had no face slot at all until v1.2.127, so there was nothing for a Select-shaped census to hold accountable before this law; now there is.",
    checkId: "faces-in-choices",
    status: "enforced",
  },
  {
    id: "R91",
    dimension: "ui",
    law: "NO NESTED SCROLL: ONE PAGE SCROLL ONLY. Aurora, verbatim, 19 Sep 2026 ~13:30–13:40: \"amke this a rule, never need to scroll to see al content!!! (only exception chat compnents)\" — stated as a standing law to be enforced everywhere. Asked directly whether the rail and the assistant pane are exceptions too, her answer, verbatim: \"exceptions like chat,\". One page scroll only; no scroll area inside a page's content. When a section or component fits its own container, nothing scrolls. When it does not, the section itself becomes a bounded scrolling region with an explicit `overflow-y: auto` / `overflow-x: auto`, never the page scrolling one region and an inner section scrolling another. EXCEPTIONS: chat components (the ticket thread's `CardContent` in `ticket-detail-body.tsx`, `AgentChat`'s own thread; L31/L33's own shape); the rail's own list of sections (pinned height, scrolls on overflow); the assistant pane (one aside scrolling region); true overlays (dialogs, sheets, popovers, select listboxes, command menus — a modal or a dropdown may scroll internally and holds no relation to the page's own scroll). NOT COVERED: horizontal scroll on tables and boards (`overflow-x-auto`). R91 is a VERTICAL law only; horizontal was never in question. **AMENDED, 21 SEP 2026: the five pending scrollers and the tables/boards question, both settled.** Five sites (the assistant's History tab, the paused-turn confirm list, the tickets dashboard's chart column, the import wizard's rejected-rows preview, and the meeting transcript preview) were shown to her on a decisions page, one recommendation each, keep or flatten; her ruling, verbatim: \"confirm\", she accepts every recommendation. KEPT, moved from pending to SANCTIONED in `NO_NESTED_SCROLL_EXEMPT`: the History tab list (`agent-history-tab.tsx`, \"same shape as the rail's own sanctioned list\"), the dashboard chart column (`tickets-dashboard.tsx`, \"a bounded dashboard widget, not the page itself\"), the import preview (`import-screen.tsx`, \"a bounded preview inside one wizard step, the same shape as a dialog body\"), and the meeting transcript preview (`meeting-detail.tsx`, \"a transcript flowing into the page would bury everything else on the record\"). FLATTENED: the paused-turn confirm list (`agent-panel.tsx`) no longer scrolls on its own; the page scrolls, and a list too long to show at once is capped by a \"Show more\" door (R88 style) rather than a second moving region beside the thread's own sanctioned scroll. Separately, asked whether a table or a kanban board is a fifth sanctioned shape of its own, her ruling, verbatim: \"like everything else\", it is not. A table or a board obeys R91 exactly as every other component does, no carve-out: a vertical scroll region inside one is an ordinary offender, checked by a second, narrower census (`findTableBoardOffenders`, same test file) against `TABLE_BOARD_SCROLL_EXEMPT` (`shared/rules/registry.ts`), empty as of this ruling because no table or board in the app currently scrolls on its own. Horizontal is untouched by this ruling: a wide table's or a board's own `overflow-x-auto` is still not this law's subject. CHECKED as a source census, `no-nested-scroll` (`web/test/no-nested-scroll.test.ts`), walking `web/components` and `shared/web` for every vertical-scroll class or style (`overflow-y-auto`/`overflow-auto`/`overflow-y-scroll`/`overflow-scroll`, and a `max-h-*` paired with an `overflow` class): every match must be named in `NO_NESTED_SCROLL_EXEMPT` with a reason, keyed by the source expression rather than a line number, rot-checked both ways so the list can only shrink, plus the table/board census above against `TABLE_BOARD_SCROLL_EXEMPT`.",
    why: "ONE OF THREE SURFACES THIS APP TEACHES A READER BY HAND, NOT CODE. The page, the ticket detail screen, and the assistant panel are the app's three scrolling surfaces. A person reads a rule by watching which region moves when she scrolls — if three regions move at once, the rule is unclear. The browser's own scroll is the page; the ticket detail's conversation thread scrolls inside its own card and nothing else moves; the assistant is the far-right column with its own scroll. Everything else sits still.",
    checkId: "no-nested-scroll",
    status: "enforced",
  },
  {
    id: "R92",
    dimension: "ui",
    law: "MAIN EXCLUDES SECONDARY: WHEN A MAIN PERSON IS CHOSEN, THEY DISAPPEAR FROM THE SECONDARY PICKER OVER THE SAME POOL. Aurora, verbatim, 20 Sep 2026: \"Generally, always when selecting main/secondary people (staff, contacts, etc.): when I select the main, this person should not be available as secondary. E.g. when I select 'Raised by,' this person should disappear from the 'Keep in the loop' options. Make this law.\" Her own examples are all TWO INDEPENDENT PICKERS over ONE SHARED POOL: raised by / keep in the loop, assignee / reviewer, account manager / members, owner / stakeholders. `withoutMain()` (`shared/web/without-main.ts`) is the one seam: a caller's secondary picker narrows its options by excluding the chosen main's id, rather than re-deriving the `.filter()` by hand at every call site. CENSUSED, off the disk, `web/test/main-excludes-secondary.test.ts`: within one file, every single-value `<Select>`/`<RecordPicker>`/single-mode `<StaffPillPicker>` bound to a MAIN-shaped field (`raisedByContactId`, `assigneeId`, `accountManagerId`, `leadUserId`, `ownerUserId`, and their prefixed siblings) is a MAIN picker; every `mode=\"multi\"` `<StaffPillPicker>` bound to a SECONDARY-shaped field (`loopContactIds`, `stakeholderContactIds`, `reviewerIds`, `memberIds`) is a SECONDARY picker; when a main and a secondary in the SAME FILE draw their options from the SAME source array, the secondary must call `withoutMain(` or an equivalent id-exclusion `.filter()`, or the pair is a finding. `MAIN_EXCLUDES_SECONDARY_EXEMPT` is the reasoned, rot-checked way out, keyed by `${file}#${mainField}+${secondaryField}`. EMPTY ON THE DAY THIS LAW SHIPPED: the census's own field vocabulary reaches the tickets lane's \"Raised by\"/\"Keep in the loop\" pair too, but it is not actually a finding — the loop is a DERIVED staff roster (raiser + admins + mentions, `workers/content/src/lib/stakeholders.ts`), never a second read of the raised-by picker's own contacts array — so the two pickers do not share a source pool and there is nothing to exclude. The census found no pair anywhere in the app drawing a main and a secondary from one shared pool without already excluding the main.",
    why: "SCOPED AWAY FROM app-form-dialog.tsx's \"Main stakeholder\" ON PURPOSE, AND THAT IS A DIFFERENT SHAPE, NOT AN EXCEPTION TO THIS ONE. An app's Main stakeholder is chosen FROM its already-ticked Stakeholders list — the main is meant to be a MEMBER of the secondary set, never excluded from it, which is the opposite relationship from raised-by/loop. This codebase's own naming tells the two apart without a hand-kept list: a field literally named `main<Something>Id` is always the \"picked from\" shape (`mainStakeholderContactId`, `mainHolder`), never the \"picked independently, then excludes\" shape her ruling names, so the census's own MAIN_FIELD vocabulary deliberately does not include a bare `main` prefix.",
    checkId: "main-excludes-secondary",
    status: "enforced",
  },
  {
    id: "R93",
    dimension: "ui",
    law: "VISUAL ACCOMPANIES TEXT: EVERY AVATAR, ICON OR COLOUR RIDES BESIDE ITS TEXT, EVERYWHERE — FILTERS, VIEWS, SELECT COMPONENTS — THE ONLY EXCEPTION BEING AN AVATAR OR LOGO INSIDE A CHIP. Aurora, verbatim, 20 Sep 2026: \"When selecting a module, also show the module's icon. Make this law: always, if there's a visual (avatar, icon or color), it should always accompany the text everywhere (filters, views, select components…), the only exception being avatars/logos in chips.\" EXTENDS R90 (`faces-in-choices`) RATHER THAN DUPLICATING IT: R90 already holds every `<Select>` choosing over a PERSON to carrying that person's own face; R93 is the sibling for a visual on an entity that is not a person — an icon or a colour — across the same three surfaces her words name (filters, views, select components). SCOPED TO MODULES ON THE DAY THIS LAW SHIPPED, her own worked example and the one population this codebase can name without guessing a type from prose: every module array is `AppModule[]` (`shared/types.ts`), carries a real `icon` field (`shared/module-icons.ts`, `DEFAULT_MODULE_ICON` the fallback), and the module's own gallery card already draws it (`modules-panel.tsx`). CENSUSED, off the disk, `web/test/visual-accompanies-text.test.ts`: every `<Select>`/`<RecordPicker>` whose options are mapped from a module-named source array must draw the module's icon (`icon=` on every `<SelectItem>`, or `icon:` in a `<RecordPicker>`'s option literal), or be named in `VISUAL_ACCOMPANIES_TEXT_EXEMPT`, keyed by `${file}#${sourceArrayExpression}`. Status and ticket/story-type colour already have their own standing censuses (`status-owns-the-chip.test.ts`, `status-stage-picker-dots.test.ts`) and are read as already satisfying this law rather than re-covered here, to avoid two laws disagreeing over one population. WIRED THE DAY THIS LAW SHIPPED: the portal's \"What is it about?\" module `<Select>` (`web-portal/components/raise-ticket-dialog.tsx`), each `<SelectItem>` now carrying `icon={<Icon name={m.icon ?? DEFAULT_MODULE_ICON} />}`.",
    why: "THE SELECT TRIGGER'S OWN CHOSEN VALUE IS A NAMED, OPEN GAP, NOT A SILENT ONE. R90's `face=` reaches both a `<SelectItem>` AND the closed `<SelectTrigger>`'s own chosen value (kit v1.2.127 gave both slots a `face` prop for exactly that reason); the kit's `<SelectTrigger>` has no equivalent `icon=` slot yet — only `face` — so a module Select today shows the icon in the open list but not in the closed field. Filed upstream rather than worked around app-side (R39: kit changes live in the kit repo only); the day the kit grows the slot, this law's own census gains a second clause for it, the same way R90's own history describes the kit growing `face` before this law could ask for it.",
    checkId: "visual-accompanies-text",
    status: "enforced",
  },
  {
    id: "R94",
    dimension: "ui",
    law: "CHIP ORDER: ID, STATUS, TYPE, MAIN PARENT, SECONDARY PARENT — ALWAYS THIS ORDER, EVERYWHERE A RECORD DRAWS A ROW OF CHIPS. Aurora, verbatim, 20 Sep 2026: \"On story detail, the chips in order: id, status, type, app (underlined), sprint (id, underlined). This must always be the order, everywhere, for other things too: 1 id, 2 status, 3 (if) type, 4 main parent, 5 secondary parent.\" ONE SEAM, `orderChips()` (`shared/web/chip-order.ts`): every chip is tagged with WHICH of the five kinds it is (`ChipKind`), and the seam sorts by that tag rather than the order a caller happened to write the JSX in — the same shape `sorted-options.ts` (R75) already takes for \"the order is the data's property, not the call site's.\" A kind simply absent from a given record (no type vocabulary, no secondary parent) is not padded; it is left out and the rest still resolve to the fixed order. WIRED THE DAY THIS LAW SHIPPED: `shared/web/ticket-chips.tsx`'s `TicketChips` (id/status/type/mainParent, already in the right order, now structurally guaranteed rather than merely correct today) and `web/components/accounts/contact-panels.tsx`'s \"tickets raised for this contact\" row — a REAL, live defect this law's own census caught: that row drew TYPE before STATUS, the two swapped from the law's own order, fixed by routing the pair through `orderChips`. CENSUSED, off the disk, `web/test/chip-order.test.ts`: any file building a chip row that carries both an id chip (`<RecordRef`) and a status chip (`variant=\"status\"` or `ticketStatusCell(`) within one reading window must route that construction through `orderChips(`, or be named in `CHIP_ORDER_EXEMPT`, keyed by `${file}#${idChipValueExpression}`.",
    why: "A STATIC CENSUS PROVES THE ROW WAS BUILT THROUGH THE ONE FUNCTION UNIT-TESTED TO RESOLVE THE FIXED ORDER; IT DOES NOT RE-DERIVE THE VISUAL ORDER ITSELF FROM SOURCE. Reading five chip kinds' relative screen position back out of arbitrary JSX, off disk, without running the component, is the kind of check that is either so loose it passes anything or so specific it breaks on the next harmless refactor — the same trade-off `pinned-toolbar`'s own class census settles by pairing a structural check with a separate live-injection proof rather than trying to compute pixel order from text. `orderChips()`'s own unit tests (this file, above) are where the ORDER claim is actually proved; the census's job is only to prove a caller reached for the seam instead of hand-writing the sequence again.",
    checkId: "chip-order",
    status: "enforced",
  },
  {
  ...
  },
  {
    id: "R110",
    dimension: "ui",
    law: "THE FILTERS OPEN AS AN OVERLAY, NEVER AS A SECOND ROW — AND THE OVERLAY PICKS ITS OWN FORM. Two rulings of Aurora's, a Chapter apart, and they only work together. 2 Sep 2026, verbatim: the filters must open as \"a temporary overlay not a second row\". 23 Sep 2026, choosing among five drawn designs: \"filter drop sheet popover\". ONE COMPONENT DECIDES, AND IT DECIDES FROM THE CONTENT: `filterOverlayForm(span, hasRoom)` (`shared/ui/components/filter-bar/filter-bar.tsx`) is a PURE FUNCTION of what the declared facets cost — never a prop, never a breakpoint, never a screen's own opinion — so a facet added to a collection tomorrow moves the form on its own, which is what her \"the sheet is simply a popover that ran out of room\" describes. It counts HEIGHT rather than heads (`FACET_SPAN`: a field costs 1, a range costs 2, because a range draws a min and a max side by side with a line kept under them for its error state), spends it against `FILTER_POPOVER_BUDGET` (3, her own \"two or three facets\" against \"four or more\"), and answers `popover` at or under the budget, `sheet` above it, and `bottom-sheet` on a phone whatever the content costs. THE ANCHOR IS THE TOOLBAR'S OWN TRACK, marked once by a shared toolbar with `FILTER_ANCHOR_ATTR` (`data-filter-anchor`) rather than a ref threaded through every screen that has filters: that attribute is what makes a drop sheet the width of the toolbar instead of the width of the Filter pill. AND NOTHING LANDS IN THE TOOLBAR'S OWN BOX. The four ways this has shipped green before are one shape — a panel nested in the pill track (a giant oval), a panel absolutely positioned but still inside the toolbar's column, a panel in NORMAL FLOW under the track pushing the collection down, and two same-toned boxes with a seam between them read as a second toolbar — so the assertion is STRUCTURAL rather than visual and is made at all three widths: the toolbar's in-flow markup is byte for byte unchanged when the overlay opens, its column still has exactly ONE child (the track), the rows below are not displaced by a node, and the overlay is a descendant of the column, of the pinned box and of the screen's own container: NONE of the three. CHECKED, `web/test/filters-open-as-an-overlay.test.tsx`, which proves the decision twice — directly, as the function (`FILTER_POPOVER_BUDGET` is 3, a field plus a field plus a range is 4 and therefore a sheet, a phone gets neither form) and then as a RENDER, that the component really asks it (`data-form` on the open dialog). It proves the surface behaves like an overlay (Escape closes it, a press on the ground closes it, focus is trapped while it is open and returns to the Filter control when it shuts), that it carries the design's own two foot controls (Clear all appears only once something is on; Show N says the live count and closes the overlay, because the filters are already applied), and that a phone gets the KIT's own bottom sheet with its side marker, its grabber and the heading a sheet must have. AND FOUR CENSUSES over every `.tsx` in `web/`, `web-portal/` and `shared/web/`, read through the shared comment stripper: nobody passes a `toolbarPanel` (the slot through which the facets used to reach normal flow — the kit's vendored `CollectionFrame` still declares one, so the law is about the APP: nothing here hands anything to that position); nobody destructures a `panel` out of `useFilterBar`, because the `{ pill, panel }` pair was the shape that made a second row possible at all and the hook returns ONE node now; every element carrying `data-slot=\"toolbar-row-track\"` carries the anchor attribute on the SAME element, derived rather than listed so `wave-finder.tsx`'s registered hand-copy is held to it too, with a tripwire that fails if the walk finds fewer than three tracks; and the overlay is reached through the KIT (`shared/web/screen-engine/filter-bar.tsx` imports `FilterOverlay` from `@shared/ui/components/filter-bar/filter-bar`), with the app-side twin `shared/web/screen-engine/filter-overlay.tsx` asserted NOT to exist — it was app-side for exactly as long as it had to be, because `shared/ui` is vendored and content-hashed and an unreleased component cannot be imported until somebody mints the tag, and kit v1.2.166 carries it (R39: a second copy of a kit part is the fault the hash exists to prevent, and a filter overlay the next app does not inherit is the whole reason the kit is the only UI input).",
    why: "THE FORM IS TESTED AS A FUNCTION AND AS A RENDER, AND NEITHER HALF WOULD DO ON ITS OWN. Asserting the threshold through a rendered width would be inferring the rule from a symptom: jsdom has no layout, so \"is this a popover?\" measured on screen is really \"did a class name change\", which is the same green a broken decision gives. Asserting only the function would prove a rule nothing has to obey. So the budget is asserted directly, where it is a number, and the component is then asked the same question through `data-form` on the open dialog. THE SECOND-ROW HALF IS STRUCTURAL FOR THE SAME REASON: every one of the four ways this shipped before was VISIBLE and every one passed its build, because what was wrong was where a node sat and no test asked. \"The toolbar's in-flow markup is unchanged, byte for byte, and the overlay is not a descendant of anything the screen renders\" is a question jsdom CAN answer, and it catches all four at once without describing any of them. AND THE CENSUSES LOOK FOR THE ABSENT SLOT RATHER THAN THE PRESENT PANEL: a filter panel in normal flow is not a thing you can pattern-match, but the PLACE that made it possible is — a `toolbarPanel` prop, and a hook that returned two nodes so a caller could put the second one anywhere. Removing the position is what makes the ruling structural instead of a habit, and the censuses are what keep the position removed.",
    checkId: "filters-open-as-an-overlay",
    status: "enforced",
  },
  {
    id: "R111",
    dimension: "ui",
    law: "A PHOTOGRAPH ALWAYS BEATS INITIALS. INITIALS ARE THE FALLBACK FOR A PERSON WITH NO PICTURE, NEVER THE DEFAULT FOR A CALL SITE THAT DID NOT GO AND GET ONE. Aurora's ruling, 23 Sep 2026, verbatim, found on the Choices table's own \"Added by\" cell and generalised by her in the same sentence before anybody could scope it to that screen: \"on choices adde by show avatar, not initials. make this a rule, but not only for this case but always: where there's avatar show it- only initials when avatar is empty.\" Wherever this app draws a PERSON — `<RecordMark shape=\"round\">` (R31's own declaration that a record is \"a person in their own right\"), `<PersonCard>`, `<PeopleFaces>`, a `RecordPicker` option, a kit `SelectItem`'s `face` — that mark is handed the person's photograph, and the initials tile renders only where there is genuinely no photograph on file. THE COMPONENT WAS NEVER THE DEFECT: `RecordMark` (`shared/web/record-mark.tsx`) has drawn a picture when given one and fallen back to an initial when not since the day it was written, so a check that read the component would have been green on the morning she filed this. The defect is upstream of it every time, and all six found the day this law shipped were the same shape — a fact the call site already held, or could reach in one lookup, and simply did not forward: `shapeChoicesTable`'s Added-by cell (`web/components/deep-link/shape.tsx`), where the door had been SELECTING `selectable_data.creator_id` since the Added columns landed, with a comment naming this exact use (\"the id is what a future face would resolve through (R35)\"), and `toValue` dropped it — fixed by carrying `createdById` on `SelectableValue` (`shared/types.ts`, `workers/tenancy/src/lib/selectable.ts`) and resolving it against the same cached members list every picker in the app already reads; the Kanban card's assignee (`web/components/work/stories-screen.tsx`), where `membersById` was already in scope, built for the Assignee facet, whose own comment says that list carries \"a picture `Story` rows themselves do not\"; the app stakeholders checklist AND its Main stakeholder picker (`web/components/apps/app-stakeholders-fields.tsx`), where `AccountLink.personLogoUrl` has carried the contact's face the whole time and BOTH callers flattened it away in their own `.map`; the task sheet's assignee chip (`web/components/work/task-sheet.tsx`), drawing a letter tile while the Effort card three hundred lines down the same file drew each logger's real face off the same members cache; and the Stakeholders tab's \"Theirs\" column (`web/components/apps/stakeholders-panel.tsx`), which hardcoded `photo: null` while \"Ours\" beside it resolved a real picture one line above. CHECKED, `web/test/photo-beats-initials.test.ts`, a CALL-SITE census over every `.tsx` under `web/components`, read through the repo's one comment stripper (the files this walks quote `<RecordMark … shape=\"round\" />` in their own prose, and a census that counted those would report four findings that are not code): every `<RecordMark shape=\"round\">` and every `<PersonCard>` that carries no `picture=` is a finding, unless the call site is named in `PHOTO_UNREACHABLE`, keyed by `{file, contains}` — a fragment of the call site's own text, never a line number — with the reason the photograph cannot be got from where it stands, rot-checked both ways so the list can only shrink. A tripwire renders a synthetic sample through the same parser and asserts it FINDS the offender, so the census cannot pass by matching nothing.",
    why: "THE BURDEN IS INVERTED, BECAUSE \"CAN THIS EXPRESSION REACH A PHOTOGRAPH?\" IS NOT A QUESTION A SOURCE CENSUS CAN ANSWER. The answer lives in a door, a cache and a type, one import graph away, and a check that tried to follow it would be a type-checker with worse judgement. So a person's mark drawn with NO picture is a finding by default and the only way out is a line SAYING why the photograph is unreachable — which turns an unanswerable question into an answerable one and puts the answer where the next reader finds it, instead of in a comment nothing reads at build time. That is the same move R29, R60 and R96 already make with their own registries, and it is what makes this stronger than a check on the component: the component is always innocent, so a component check is always green. THE TABLE IS NOT EXPECTED TO BE EMPTY, unlike most in this repo, and that is deliberate rather than slack — a person with no photograph on file is the ordinary case and some marks stand for somebody who does not exist as a record yet at all (`shapeInvitesList` draws a PENDING INVITE, whose only identity is the email it was sent to: no user row, no `image_url`, nothing to resolve one through until somebody accepts). What the table buys is the DIFFERENCE being written down: \"this person has no picture\" (fine, the mark falls back, which is what she asked for) versus \"this screen never went and got the picture\" (the defect, six times over). THE ONE THING THIS CENSUS CANNOT SEE is a call site that passes `picture={…}` with a value that is hardcoded `null` — `stakeholders-panel.tsx`'s \"Theirs\" column was exactly that and was found by reading, not by the check. A census over VALUES rather than prop names would need to resolve an expression, which is the same wall the paragraph above describes; the honest statement is that this law catches the missing ASK and a reader still has to catch a dishonest answer.",
    checkId: "photo-beats-initials",
    status: "enforced",
  },
  {
    id: "R112",
    dimension: "arch",
    law: "ARCHIVED MEANS INVISIBLE, AND IT REACHES DOWN. Aurora, 23 Sep 2026, validating the account round and widening it in the same breath, verbatim: \"validated - this for everything when archived, not only accounts.\" Archiving is not a fact about a company; it is a fact about ARCHIVING, and it reaches the rows that hang off whatever was archived, whichever record type that is. WHICH RECORD TYPES CAN BE ARCHIVED IS READ, NEVER GUESSED: exactly two tables in the team schema carry the archive quartet (`archived_at` + `archiver_id`/`archiver_email`/`archiver_name`) — `help` (migration 0011, a ticket somebody put away) and `accounts` (migration 0117) — and `ARCHIVABLE` (`workers/content/src/lib/help.ts`) is that list as DATA, so a third record type costs one line rather than a second mechanism. ONE CLAUSE BUILDER FOR BOTH, `archivedParentClause({ parent, table, column })`, which is the account version's own body parameterised rather than a rival to it (`accountArchivedClause` is now a one-line wrapper over it, emitting byte-identical SQL): a correlated `EXISTS` with its OWN per-parent alias (`arc` for accounts, `arh` for help), never a bare `archived_at` — a bare one is refused by SQLite at runtime with \"ambiguous column name: archived_at\" and, when it happened on `help`, took the WHOLE Tickets screen down rather than the three doors it was aimed at — and a `<pointer> IS NULL` short-circuit FIRST, because a row whose pointer is null has no parent's archived state to inherit: four out of five stories carry no ticket (migration 0014's own header on the nullable `ticket_id`) and most tickets carry no account, so a clause that dropped the short-circuit would empty a board rather than filter it. EVERY DOOR THAT LISTS A CHILD ASKS IT, and the badge asks the identical question the rows answer (R16): `storyWhere` AND `countStoryViews` (`workers/content/src/lib/stories.ts`, the per-tab badges build their own WHERE by hand and are the half most likely to drift), `whereFor` AND the counts read (`workers/content/src/lib/todos.ts`), and `ARCHIVED_PARENT_HIDES` (`workers/content/src/lib/record-map.ts`), a MAP from a table to the archivable parents it hangs off rather than the set of one it replaced, because a story hangs off two. THE KNOWLEDGE BASE RETIRES ITS MIRRORED COPIES RATHER THAN MERELY NOT SERVING THEM: a reader that filtered archived rows out of its own sweep would never visit them again, so the chunks already embedded would go on answering questions for ever with no row left to walk back — every reader SELECTS the parent's archived state under its own alias (`account_archived_at`, `ticket_archived_at`) and hands it to `retired`, and bumps its own `textVersion` so the cursor walks back over the rows already filed. THE DOOR THAT IS ALLOWED TO SEE IT STILL OPENS IT — the ticket's Archive view, the accounts Archived tab, a record's own map — because you cannot take a record out of a drawer you can no longer reach into, and STAFF ARE NEVER REFUSED, or an archived account could never be restored. AND THE OTHER END OF THE SAME SENTENCE, which she settled the same day: portal login is refused for the contacts of an archived account, answered in `resolveAccountScope` (`shared/workers/account-scope.ts`) where a revoked grant is ALREADY refused — standing nowhere, seeing nothing — never a new gate the next fenced door would have to remember. `deactivated_at` IS NOT THIS and may never be folded into it: thirty-two tables carry it and it is her INACTIVE, her own distinction the same week (\"inactive have their own tab - but do not show in choice components but i can still see them and acces everything underneath, archived however are completley invisible\"). CHECKED, `workers/content/test/archived-hides-its-children.test.ts`, against a REAL SQLite database running the real team migrations: which tables carry the quartet is read off `pragma_table_info` rather than trusted, the deactivatable tables are asserted NOT to hide anybody's children, an archived ticket's story and to-do leave every list and every count while the no-ticket controls stay, unarchiving hands both straight back, the map drops the ticket and its story while keeping the enabler story, the Archive view still opens the ticket, archiving one record never writes the other's column, the corridor resolves a contact into their company and then into NOTHING once it is archived while a staff caller stays staff, and each reader is asserted to RETIRE on its parent's aliased column rather than merely select it. **AMENDED 24 SEP 2026, MIGRATION 0123 — FROM HIDING TO CASCADING.** Aurora read the first landing back and widened it, verbatim: \"wdym by vanish? should stay in the system, but invisible. just in case we need to in the future recover it. if ticket archive - story archived as well\" and \"when archiving a parent item, always archive as well the child items.\" Filtering was the wrong shape: a child whose parent was archived was invisible but its own row was untouched, so it was findable nowhere and recoverable as nothing. A CHILD NOW BECOMES ARCHIVED ITSELF. Eight more tables gained the audit four (`apps`, `meetings`, `tasks`, `todos`, `stories`, `sprints`, `waves`, `processes`), and those eight plus `accounts` and `help` gained a MARKER, `archived_via_table`/`archived_via_id`, because a timestamp cannot tell a child the cascade took from one somebody archived on its own merits — and un-archiving must restore the first and walk past the second. Three states, no value overloaded: `archived_at IS NULL` is live, set with a NULL marker is \"somebody archived THIS\", set with a marker is \"the cascade from that row did it\". THE OWNING GRAPH IS DATA, NOT THE FOREIGN KEYS (`shared/workers/archive-cascade.ts`): thirty-two foreign keys point at `accounts`, and `stories.ticket_id` and `help.raised_by_contact_id` are the same shape in `pragma_foreign_key_list` and opposite answers in the product, so every edge is a decision somebody made. Her four rulings of 24 Sep 2026 are its four clauses, verbatim: an APP is an owning parent (\"yes, archiving th eparent archive the child\"); a SPRINT is not (\"no. only archiving app or account would archive stories\"); a CONTACT is not (\"no\"); and `work_logs` is NEVER a target at any depth, from any parent (\"never archive work logs, time is logged and we must always know where it went\") — written at the place that decides, as `WORK_LOGS_ARE_NEVER_ARCHIVED`, and asserted as a NEGATIVE because an absence from a table is indistinguishable from an oversight. IT TERMINATES FOR FREE: every step is `UPDATE … WHERE archived_at IS NULL RETURNING id`, so an already-archived row returns nothing and there is nothing to recurse into — THE PREDICATE IS THE VISITED SET, a diamond is visited once, and first writer wins. The one self-edge (`accounts.parent_account_id`) is proven acyclic at its own write door. NOTHING VISIBLE EVER HANGS UNDER SOMETHING INVISIBLE: restoring a row whose owning parent is still archived is refused, 409 `parent_archived`. AND THE HONEST LIMIT, said rather than buried: D1 gives no transaction across statements, so a crash after the parent row moved leaves a partial state that R17 will not let a re-run finish — the recovery is restore, then archive, and the suite proves exactly that rather than claiming atomicity. The filter this law shipped with STAYS, as the belt for the one case a cascade cannot reach: a child created AFTER its parent was archived is born live under an invisible parent. Proved by `workers/tenancy/test/archiving-cascades.test.ts` beside the original check.",
    why: "A SOURCE CENSUS CANNOT SEE THIS LAW'S WORST FAILURE, WHICH IS WHY THE CHECK RUNS REAL SQL. The clause is assembled at runtime from three files and interpolated into a JOIN, so the ambiguity that took the Tickets screen down is invisible to any text scan at the site of the join — only running the real statements against a real database can see it, and `workers/tenancy/test/ambiguous-join-columns.test.ts` exists beside this precisely because the static census cannot reach that shape. THE KNOWLEDGE HALF IS ASSERTED POSITIVELY — the reader must SELECT the parent's state and RETIRE on it — rather than as \"must not select archived rows\", because the negative form is satisfied by exactly the wrong fix: filtering them out of the sweep looks correct, is cheaper, and strands the embedded copy for ever with nothing left to walk back. AND THE PORTAL HALF IS ASSERTED AT THE CORRIDOR RATHER THAN AT A DOOR, which was learned by breaking it: with the refusal deleted, every door assertion in this suite stayed GREEN, because `ticketWhere` already filters archived accounts and an empty list is the same empty list whether the fence refused the caller or the clause hid the rows. An outcome test would have pinned nothing.",
    checkId: "archived-hides-its-children",
    status: "enforced",
  },
]
...
/** R91 (`no-nested-scroll`) — THE REASONED, ROT-CHECKED WAY OUT for a vertical
 * scroll container (`overflow-y-auto`/`overflow-auto`/`overflow-y-scroll`/
 * `overflow-scroll`, or a `max-h-*` paired with an `overflow` class) found by
 * `web/test/no-nested-scroll.test.ts`'s own census of `web/components` and
 * `shared/web`. Keyed by `<file path relative to the repo root>#<the
 * className string it was found in>` — the EXPRESSION, never a line number,
 * because a file:line key rots on every edit above it and this file in
 * particular (`ticket-detail-body.tsx`) is being actively edited by another
 * lane the same day this table was written.
 *
 * FOUR SANCTIONED SHAPES, drawn straight from the law's own exception list:
 * a chat component's own thread (the ticket conversation's `CardContent`),
 * the rail's own nav list, and a true overlay's body (a dialog, a sheet, a
 * popover, a select listbox or a command menu — every dialog/sheet/popover
 * entry below). EVERYTHING ELSE this census found on the day the law shipped
 * — six sites, none of them chat, rail or an overlay — is named here with the
 * SAME placeholder reason, "pending Aurora's answer on tables and boards, 20
 * Sep 2026," so the gate stays green while her call on question 4 (Round 27,
 * tables and boards) is still open; the lane's own report lists every one of
 * them as a decide item rather than silently declaring them fine. Rot-checked
 * both ways: a line naming an expression the file no longer carries has
 * outlived its subject and fails the build, same discipline as every other
 * `_EXEMPT` table in this file. */
export const NO_NESTED_SCROLL_EXEMPT: Record<string, string> = {
  // ── SANCTIONED: chat components ─────────────────────────────────────────
  "web/components/tickets/ticket-detail-body.tsx#min-h-0 flex-1 overflow-y-auto p-4":
    "the ticket conversation thread's own `CardContent` — the law's own named exception (`AgentChat`'s own thread and this card are the two sanctioned chat scrollers). The thread scrolls inside its own card; nothing else on the page moves with it.",

  // ── SANCTIONED: the rail's own nav list ─────────────────────────────────
  "web/components/shell/app-shell.tsx#min-h-0 flex-1 overflow-y-auto overflow-x-clip":
    "the rail's own list of sections — pinned height, scrolls on overflow, the law's own named exception. Wraps `<Rail>` directly; nothing else in the shell scrolls with it.",

  // ── SANCTIONED: true overlays (dialog / sheet / popover bodies) ─────────
  "web/components/screens/automation-edit-sheet.tsx#flex-1 overflow-y-auto overscroll-contain px-6 py-5":
    "a dialog body — the automation edit sheet's own scrolling content region, below its fixed head. A true overlay, the law's own named exception.",
  "web/components/team/access-tokens.tsx#bg-popover min-h-0 flex-1 overflow-y-auto":
    "a sheet (drawer) body — `bg-popover` matches the drawer's own surface (sheet.tsx: \"Overlay surface is --popover\"), the head and foot stay fixed and only this region scrolls. A true overlay, the law's own named exception.",
  "web/components/knowledge/google-source-dialog.tsx#flex max-h-56 flex-col overflow-y-auto rounded-[var(--radius)] bg-surface-panel":
    "a dialog body — the Google source picker's own options list, inside `GoogleSourceDialog`. A true overlay, the law's own named exception.",
  "web/components/knowledge/google-scope-dialog.tsx#flex max-h-56 flex-col overflow-y-auto rounded-[var(--radius)] bg-surface-panel":
    "a dialog body — the Google scope picker's own options list, inside `GoogleScopeDialog`. A true overlay, the law's own named exception.",
  "web/components/shell/app-shell.tsx#max-h-[85svh] overflow-y-auto md:hidden":
    "a sheet body — the phone-only \"All sections\" bottom sheet (`<SheetContent side=\"bottom\">`). A true overlay, the law's own named exception.",
  "shared/web/form-shell.tsx#overflow-y-auto overscroll-contain px-6 py-5 shadow-[var(--hairline-over)]":
    "a sheet body — every form in the app renders through `FormShell`, which slides in as the kit's `Sheet` (R59); this is that sheet's own scrolling content region, below its fixed title/subtitle head. A true overlay, the law's own named exception.",
  "shared/web/screen-engine/screen-renderer.tsx#bg-card fixed z-50 flex flex-col gap-4 overflow-y-auto p-6 shadow-xl data-[state=open]:animate-in data-[state=open]:fade-in-0":
  ...
  // flatten); her ruling, verbatim, 21 Sep 2026: "confirm", she accepts
  // every recommendation below. Four were recommended keep and move here;
  // the fifth (the paused-turn confirm list, agent-panel.tsx) was
  // recommended flatten and its scroll class is gone from the file, see
  // that component's own R91 comment, not this table.
  "web/components/assistant/agent-history-tab.tsx#min-h-0 flex-1 overflow-y-auto":
    "SANCTIONED, Aurora's ruling \"confirm\", 21 Sep 2026, over the recommendation \"keep. Same shape as the rail's own sanctioned list, a pinned-height column that scrolls on overflow.\" The assistant panel's own History tab list of past conversations.",
  "web/components/tickets/tickets-dashboard.tsx#absolute inset-0 flex flex-col gap-2 overflow-y-auto":
    "SANCTIONED, Aurora's ruling \"confirm\", 21 Sep 2026, over the recommendation \"keep. A bounded dashboard widget, not the page itself.\" The tickets dashboard's own stacked-bar chart column.",
  "web/components/screens/import-screen.tsx#max-h-48 overflow-auto rounded-[var(--radius)] bg-surface-panel":
    "SANCTIONED, Aurora's ruling \"confirm\", 21 Sep 2026, over the recommendation \"keep. A bounded preview inside one wizard step, the same shape as a dialog body.\" The import screen's rejected-rows preview.",
  // THE MEETING TRANSCRIPT'S OWN SCROLLER IS GONE, 23 Sep 2026, and so is its
  // line here. It was sanctioned on 21 Sep ("keep. A transcript flowing into
  // the page would bury everything else on the record"); her one-page design
  // answers the same worry a different way — the transcript FADES out at about
  // ten lines with the document one click away (`MeetingTranscriptSection`,
  // meeting-detail.tsx). `overflow-hidden` clips and does not scroll, so that
  // section is outside R91's subject entirely rather than excused from it, and
  // this table's own rot check is what would have caught the stale line.
}

/** R91: TABLES AND BOARDS OBEY ONE PAGE SCROLL LIKE EVERYTHING ELSE. Aurora's
 * ruling, 21 Sep 2026, verbatim: "like everything else", a table or a kanban
 * board is not a fifth sanctioned shape beside chat/rail/assistant/overlay;
 * a vertical scroll region inside one is an ordinary R91 offender. Read by
 * `web/test/no-nested-scroll.test.ts`'s own second census
 * (`findTableBoardOffenders`), keyed the same way as `NO_NESTED_SCROLL_EXEMPT`
 * (by the source expression, never a line number), and rot-checked both
 * ways so the list can only shrink.
 *
 * EMPTY, AND EMPTY IS THE GOAL. The census this table backs was written
 * against the app as it stands on 21 Sep 2026: the kit's own `Table`,
 * `DataTable` and `Kanban` each carry an OPTIONAL internal vertical scroll
 * (`data-table.tsx`'s `maxHeight` prop on its scroll shell; `kanban.tsx`'s
 * `columnMaxHeight` prop on each column), and no call site in `web/` or
 * `shared/web` passes either: every table and every board in this app is
 * content-sized, and the page scrolls. A line here would mean a table or
 * board earned a real exception under R91's own named shapes (an overlay
 * body holding a table, say); it does not mean a call site turned
 ...
 * screen-bits.tsx`), keyed by the file's path relative to the repo root.
 * Empty is the goal, and it is empty as of 18 Sep 2026: the two sites this
 * law was written over (`help-detail.tsx`'s Related stories and Work logs
 * panels) both adopted the shell rather than needing a line here — the
 * whole point of the client's ruling is that "this button stays reachable
 * on an empty collection" is no longer an argument this registry accepts.
 * `web/test/empty-state-single-door.test.ts` checks both ways: a line
 * naming a file whose `<AddButton>` now sits inside `<EmptyGatedPanel>` (or
 * that carries no such tag any more) has outlived its subject and fails the
 * build, same discipline as every other `_EXEMPT` table in this file. */
export const EMPTY_STATE_SINGLE_DOOR_EXEMPT: Record<string, string> = {
  "web/components/team/access-tokens.tsx":
    "the header row itself is wrapped `{tokens.length > 0 && (…)}` — hand-rolled rather than built through `EmptyGatedPanel`, but the SAME behaviour: at zero tokens the whole row (title-adjacent `<AddButton>` included) draws nothing at all, and the register below carries the one first-add alone. Predates `EmptyGatedPanel` (this file's own R50 comment names the identical fix, `AddButton returns null on empty… and the row goes with it`); the census reads position and cannot see a hand-rolled wrapper's condition matches the collection's real row count the way it can see `EmptyGatedPanel`'s own prop.",
...
export const TOOLBAR_EXEMPT: Record<string, string> = {
  ...
  // THE LOGS DASHBOARD'S OWN ROW STOOD HERE (R48 census ii, added 23 Sep 2026)
  // AND IS DELETED, 24 Sep 2026. It argued that a tab of grouped pictures has
  // nothing for a search box to sieve, which is still true — and Aurora ruled on
  // the LOOK rather than the logic: "add full toolbar, even if search is
  // diasbled." `web/components/work/time-screen.tsx` passes a real `search` now,
  // the kit's own `SearchInput` in the kit's own disabled state, so the census
  // no longer finds this file missing the prop and an entry left here would fail
  // R48's own rot-check. The list shrinks, which is the only direction it may
  // move.
...
export const UNCONTAINED_SECTION_OK: Record<string, string> = {
  // EMPTIED 21 SEP 2026, AND THE TABLE KEPT. Every line that stood here was a
  // section standing on the bare page ground with a reason for the debt. The
  // debt is not a debt any more: Aurora took the boxes off the whole app that
  // day ("go an implement this appwide", rulebook L43), so a section on the
  // page ground is the DEFAULT and there is nothing left to excuse. The eleven
  // entries were deleted rather than re-worded, by this table's own rot-check,
  // which has always said a line matching nothing must go.
  //
  // WHAT THE TABLE IS NOW. R67's surviving clause is "a section paints the
  // page or the kit's paper - never a stroke, never a hex", and this is the
  // reasoned way out of THAT. It is empty on the day it was repointed, which
  // is the honest state: no section in either front door draws an outline or a
  // hex, and if one ever has to, the reason belongs here rather than in a
  // comment nothing reads at build time.
}

// ── PAPER ON PURPOSE — THE CENSUS R67 BECAME WHEN L43 WENT APP WIDE ───────
//
// Aurora, 21 Sep 2026, on the Minimal Kit page, verbatim: *"board A / space 6
// / go an implement this appwide, also implement the to the bottom edge for
// main content and assistant like in your previous artifact / also, make
// footer not inside a container, but the full row side to side (within the
// main content) / do think a lot about each component, what this minimalising
// means so that it still works"*, and her goal for the whole pass, in her own
// words: *"the goal: make a more minimal clean app"*.
//
// THE CENSUS TURNED ROUND. Until this ruling the app's question was "which
// section is NOT on paper, and why is that allowed" — `UNCONTAINED_SECTION_OK`
// above is the arrears against that. Plain is the default now: every
// collection frame, every titled side section and every module's grouping
// panel paints nothing, through the shells' own defaults (`CollectionCard`,
// `TicketSidePanel`, `EmptyGatedPanel`, `Panel`). So the interesting list is
// the opposite one — which surfaces still paint soft paper, ON PURPOSE, and
// what each of them is that a grouping section is not.
//
// FIVE THINGS KEEP THEIR PAPER, and the taxonomy is hers and the Minimal Kit
// page's, not one invented here:
//
//   · A CONVERSATION CARD. "the conversation card stays a card."
//   · AN EMPTY OR ERROR STATE. The page's own finding, validated: "The empty
//     state keeps its paper… It is the one register on the page whose whole
//     job is to draw a boundary around nothing, and a boundary around nothing
//     with no boundary is just nothing."
//   · A TILE. Her word for one, the same day: "this is a metric, like in kit"
//     — and the kit's own `StatGrid` tile is a `Card variant="default"`.
//     A per-record card in a grid and a board card are the same kind of
//     object: a thing you can pick up, not a region you read.
//   · A WELL. The kit's chapter 13: "a well holds secondary detail inside a
//     card… same radius, no edge, no shadow." Already its own variant.
//   · A NOT-A-SECTION. A dialog or sheet body, a preview that DRAWS a card as
//     its subject, a composer, an inverse band.
//
// KEYED BY FILE, with the reason in her words or the page's. A file appears
// once however many such cards it draws; the reason says which. Rot-checked
// both ways by `web/test/plain-surface-scope.test.tsx` (`PAPER_ON_PURPOSE`):
// a paper-painting `<Card>` in a file that is not here fails the build, and a
// file here that no longer paints one fails it too, so the list can only be
// true. `variant="well"`, `"raised"`, `"inverse"` and `"brand"` are outside
// the census: none of them is the grouping surface this ruling took the box
// off, and each already says what it is in its own name.
export const PAPER_ON_PURPOSE: Record<string, string> = {
  // ── web/ ────────────────────────────────────────────────────────────────
  ...
  "web/components/records/record-week.tsx":
    "one tile per entry in the Mon-Fri week board. A tile, by the same reading as a metric: \"this is a metric, like in kit\".",
  "web/components/accounts/accounts-screen.tsx":
    "the Accounts gallery's per-account card — a per-record card in a grid, the object a reader picks up rather than a region they read. Soft paper since 21 Sep 2026, because its ground is the plain page now and `raised` there would have measured 1.000 against it.",
  "web/components/accounts/contacts-screen.tsx":
    "the Contacts gallery's per-contact card — a per-record card in a grid, the same object and the same tone change as the Accounts wall beside it, for the same reason: on the plain page `raised` is the page's own colour.",
  "web/components/apps/apps-screen.tsx":
    "the Apps gallery's per-app card — a per-record card in a grid. The board beside it draws the kit's own lanes and raised cards (option A) and is not this card.",
  ...
  "web/components/accounts/accounts-dashboard.tsx":
    "the three KPI tiles (active accounts, countries, median tenure), and they are the Effort tiles above by construction rather than by resemblance — Aurora, 23 Sep 2026, of this tab: \"the cards kpi need some kind of background, like effort.\" Same `Card variant=\"default\"` around the same bare `StatGrid`. A TONE AND NOT A BOX, which is what keeps this compatible with the kit's own §2.8 the same day (\"by rule no borders nowhere in the kit\"): soft paper is a fill, and nothing here draws a stroke. Every OTHER surface on the tab is still plain — the two splits, the arrivals line, and the error and empty registers, which are `variant=\"plain\"`.",
  "web/components/work/logs-dashboard.tsx":
    "the four Logs dashboard figures (hours this week, hours today, logged nothing last week, records worked on). Aurora, 24 Sep 2026: \"kpi need background\" — and the background she means is the one `effort-card.tsx`'s own three tiles already paint, which is why this is the identical `<Card variant=\"default\"> + <CardContent> + <StatGrid surface=\"bare\">` rather than a second treatment that nearly agrees with it. A metric tile is one of the five things a grouping section is not (a number standing for itself), the same reading the Effort entry above carries. Everything else on the tab — the donut, the two bar ranks, the weekly chart, the top-records list — stays plain, so only the tiles keep paper.",
...
export const TOOLBAR_SORT_EXEMPT: Record<string, string> = {
  ...
  "web/components/work/time-screen.tsx#TimeScreen":
    "THE LOGS DASHBOARD'S OWN ROW, and a dashboard has no row order to offer. Aurora's ruling, 23 Sep 2026, verbatim: \"add toolbar w filters by person, account\" — two filters, named, and nothing else. What the tab draws is six GROUPED PICTURES (a donut by related record type, a line of eight weeks, two bar ranks and a top-records list), each of which already carries its own order as part of what it means: a donut ordered by share, a line ordered by time, a rank ordered by hours. A sort chip over that would have to pick one picture to reorder and leave the other five, which is a control that answers a question nobody asked of the tab. The ENTRIES tab beside it is the list of rows, and it passes a real `sorts` menu to its own `<PagedFind>` (started / duration / person, the door's own `WORK_LOG_SORTS`) — which is why this screen looks sorted where it is a list and unsorted where it is pictures, the same split `sprints-screen.tsx` states two entries down.",
...
export const RECORD_TABS_SINGLE_PANEL: Record<string, string> = {
   ...
   * (`ACTIVITY_GATE_MAP`). */
  "meeting-detail":
    "Aurora's ruling, 23 Sep 2026, verbatim: \"meetings: implement the one " +
    "page, love how you did it.\" The meeting record moved onto the same " +
    "one-page chassis a ticket and a story already stand on — " +
    "`<RecordScreen panelVisible={false} footerVisible={false}>` for the head, " +
    "`<RecordDetailBody>` for the two columns, `<ScreenFooterSlot>` for the " +
    "band — so there is no strip left for this census to find. What the four " +
    "tabs held is placed rather than dropped: Agenda and the transcript are " +
    "the main column, Attendees (the calendar tab's guest list, her word) and " +
    "Time log and Connections are the side column, and the Overview/Details " +
    "list is struck at her own ruling (\"do not include section details (its " +
    "unecessry)\"). The footer clause below still applies and is still met: " +
    "this screen hands `RecordFooterBand` its own `audit` block.",
  "member-screen":
  ...
  "story-detail":
    "Aurora's design review, 21 Sep 2026, over the story-detail-design.html " +
    "artifact: a story is what we do, built the SAME SHAPE the ticket already " +
    "is — help-detail's own ruling on this list, below, quoted there " +
    "in full. No tab strip: Detail, Acceptance criteria and Build notes stack " +
    "in the left column, Assigned to / Related tickets / Related stories / " +
    "Phase and wave / Effort / Metrics stack in the right, all drawn AT ONCE " +
    "through RecordDetailBody (web/components/records/record-detail-body.tsx, " +
    "the same R89 shape TicketDetailBody proves, extracted for this second " +
    "caller), never behind a click. `<RecordScreen panelVisible={false}>` " +
    "draws the head only; the body renders as its SIBLING, the identical " +
    "shape help-detail's own entry describes. Activity stays off the ink " +
    "footer's own eyebrow, never a tab — nothing here was ever named as one.",
  "help-detail":
...
export const RECORD_TAB_COUNT_EXCEPTIONS: Record<string, string> = {
  ...
  // RENAMED FROM `meeting-detail.notes`, 23 Sep 2026, when the tab's own value
  // moved with its word (Aurora: "on meetings: rmeove notes (we have transcript
  // for that)"). The reason is unchanged apart from losing the half that is
  // gone: it is still one record's body, and a body is not a collection.
  "meeting-detail.agenda":
    "the agenda we set, and under it the transcript of what was actually said — this module's own prose. One record's body, not a collection.",
...
export const GONE_ON_PURPOSE: Record<string, string> = {
  "shared/web/screen-engine/filter-overlay.tsx":
    "the app-side twin of the kit's own `FilterOverlay`, written on 23 Sep 2026 and deleted the same day. It existed for exactly one reason: `shared/ui` is a vendored kit, pinned and content-hashed, so the component could not be imported here until the tag carrying it was minted. v1.2.166 carries it, the adapter (`shared/web/screen-engine/filter-bar.tsx`) imports it from `@shared/ui/components/filter-bar/filter-bar`, and the file is gone. `web/test/filters-open-as-an-overlay.test.tsx` names this path precisely BECAUSE it is gone: it asserts the file does NOT exist, which is what keeps a second copy of a kit part from growing back (R39: the kit is the only UI input, and an app-side fork of a kit component is the fault `shared/ui`'s own hash exists to prevent).",
...
export const RAW_DATE_EXEMPT: Record<string, string> = {
  // GONE 23 Sep 2026 — this pinned `MeetingsAgenda`'s own `dateTime` field, and
  // Aurora's ruling ("meetings: remove agenda view.") deleted that shaper with
  // the view it fed. Nothing in meetings-screen.tsx hands a raw stamp to
  // anything any more, so the entry has nothing left to excuse and this table's
  // own rot check is what said so.
...
// ── id-chip-is-black (R96) ───────────────────────────────────────────────────

/** A still-open id-chip-is-black finding, reported rather than fixed here,
 * keyed by WHAT THE LINE SAYS rather than by "path:line" (the same shape
 * `RefAsString` above takes, for the same reason: a pin that only moves
 * whenever an unrelated line is added above it does not identify a call
 * site). A `contains` fragment matching more than one line in its file is
 * refused by the check's own ambiguity guard. */
export interface IdChipExempt {
  file: string
  contains: string
  why: string
}

/** THE BACKLOG LIST'S OWN THREE SITES, FIXED 20 Sep 2026. `web/components/
 * work/stories-screen.tsx`'s lane finished, and `storyLead()`, `ReviewsQueue()`
 * and the standalone `ID` column (`PLANNED_BACKLOG_COLUMNS` /
 * `REVIEWS_LIST_COLUMNS`) all route through `<RecordRef>` now — the badges
 * directly, the column through `TableColumn.render` (`record-table.tsx`),
 * keyed off `f.column === "ref"`, so `shapeStories()`'s own row shape still
 * carries `ref` as the raw string `rawRows`' search/sort already reads.
...
export const DEAD_EXPORT_OK: Record<string, string> = {
  ...
  "shared/rules/registry.ts::DEAD_EXPORT_OK":
    ...
    "is ever removed.",
  "web/components/work/moscow-field.tsx::moscowField":
    "MoSCoW is parked. Aurora, verbatim: \"pause everything to do with moscow, but remind me at later " +
    "stages.\" This is the field config the parked MoSCoW picker rendered through, kept in place so the " +
    "surface can be wired back up without rebuilding it, rather than deleted and re-authored the day the " +
    "pause lifts. Delete this line only once the field is reconnected, or delete the export if the pause " +
    "becomes permanent.",
  "web/components/work/moscow-filters.tsx::MOSCOW_RANK":
    "MoSCoW is parked, the same ruling as the line above. This is the rank table the parked MoSCoW sort " +
    "and filter used to order Must, Should, Could and Won't, kept for the day the pause lifts rather than " +
    "recomputed from scratch then. Delete this line only once the ranking is reconnected, or delete the " +
    "export if the pause becomes permanent.",
}
...
export const NOT_A_WORK_PICKER: Record<string, string> = {
  ...
  "meetings/meeting-detail.tsx":
    "the meeting record's own `membersQ` (R111, Aurora 23 Sep 2026: \"where there's avatar show it- only initials when avatar is empty\") — read ONLY so the Attendees section can resolve a colleague's photograph through `memberFace(membersQ.data, known?.memberUserId)`, a `.find()` by userId exactly like `tickets-collection.tsx`'s raiser avatar below and `task-sheet.tsx`'s work-log rows above. A meeting's attendee list is Google's own `attendees` array, mirrored onto the row; nothing on this screen lets a person PICK a member, and the only member-shaped choices it offers (the account, the app, the meeting type) come off their own caches. Read at all only when the meeting has guests to resolve.",
...
export const PARKED: Record<string, string> = {
  ...
  "work/moscow-chip":
    ...
    "stages.\" Parked, not dead: `Story.moscow`/`MOSCOW_VALUES` (shared/types.ts) and the create/update doors " +
    "keep accepting the field untouched, only this DISPLAY moved out of `stories-screen.tsx` and " +
    "`story-detail.tsx` into a file of its own and stopped being mounted. Delete this line and wire " +
    "`<MoscowChip>` back into both call sites the day she asks for MoSCoW back.",
  "work/moscow-field":
    "the MoSCoW priority field (`MoscowField`, a segmented Must/Should/Could/Won't control) the story form used " +
    "to draw between the assignee picker and the Category group. Same 21 Sep 2026 ruling as `work/moscow-chip` " +
    "above. Parked, not dead: `story-form-dialog.tsx` still carries `values.moscow` through an edit unchanged, " +
    "only the control that lets somebody SET it is unmounted. Delete this line and wire `<MoscowField>` back " +
    "into `story-form-dialog.tsx` the day she asks for MoSCoW back.",
  "work/moscow-filters":
    "the stories screen's own MoSCoW facet and sort option (`moscowFacet`/`moscowSortOption`), filtering and " +
    "ordering the backlog by priority. Same 21 Sep 2026 ruling as the two entries above. Parked, not dead: the " +
...
export const COLOURED_CHIP_OK: Record<string, string> = {
  ...
  // "web/components/work/stories-screen.tsx#MoscowChip" STOOD HERE, AND WAS
  // DELETED 21 SEP 2026, not because R86 changed its mind about the chip
  // being a legitimate priority colour, but because the chip itself moved.
  // Aurora's ruling that day paused every MoSCoW surface ("pause everything
  // to do with moscow, but remind me at later stages"): `MoscowChip` now
  // lives in `web/components/work/moscow-chip.tsx`, PARKED (unmounted,
  // `PARKED["work/moscow-chip"]`, below), and that file is not one of this
  // census's own `FILES`, so no entry here answers for it while it is
  // parked. Re-add a line naming the new path if `moscow-chip.tsx` is ever
  // added to `FILES` before the chip is wired back in.
}
...
/* ════════════════════════════════════════════════════════════════════════
   ...
   moment either one is not that element's own last rendered child. D21 is
   narrower than R89 on purpose and stays that way after R89's own round-24
   correction: D21 asks only whether a footer is the LAST child of ITS OWN
   CARD (DOM order), never where that card sits on the screen — the question
   R89 answers. `TicketConversationPanel` (`web/components/tickets/
   ticket-detail-body.tsx`), UN-RETIRED round 24 exactly its round-22 shape,
   is the one call site the census sees today, `<CardFooter>` still its
   `Card`'s own last child. THE BAND (Latest activity + Record, R89's own
   round-24 subject) is a DIFFERENT component entirely — `RecordFooterBand`
   calls the kit's `RecordDetail` directly, never `<CardFooter>`/
   `<ReplyComposer>` — so D21's own census does not reach it and was never
   meant to: R89 alone answers whether the BAND sits at the screen's bottom.
   This list opens EMPTY. A reasoned entry here is for a footer that
   genuinely is not a screen's own bottom-most element (a card nested inside
   a larger scrolling region, say), never for one that is merely
   inconvenient to move. */
export const FOOTER_IS_LAST_EXEMPT: Record<string, string> = {}

// ── R90 (faces-in-choices) ──────────────────────────────────────────────────

/** R90 — a `<Select>` block this census marked identity-bearing (its options
 * array's own field names name a record: `personName`/`contactId`/
 * `memberId`/`accountId`/`avatar`/`photo`/`initials`) with at least one
 * `<SelectItem>` carrying no `face=`. Keyed `${file path relative to repo
 * root}#${the source array expression(s) feeding the block}` — an
 * EXPRESSION, never a line number, so the key does not rot on an edit above
 * it. Rot-checked both ways: a line naming a block the census no longer
 * finds (fixed, or the source changed shape) has outlived its subject and
 * fails too, so the list can only shrink.
 *
 * EMPTY ON THE DAY THIS LAW SHIPPED (19 Sep 2026): every `<Select>` the
...
/** R92 — a secondary picker (a `mode="multi"` `StaffPillPicker` bound to a
 * loop/stakeholder/reviewer/member field) drawing from the SAME source array
 * as a main picker (a single-value Select/RecordPicker/StaffPillPicker bound
 * to a raisedBy/assignee/accountManager/lead/owner field) in the same file,
 * with no `withoutMain()`/exclusion `.filter()` between them. Keyed
 * `${file path relative to repo root}#${mainField}+${secondaryField}` —
 * both field names, never a line number. Rot-checked both ways: an entry
 * naming a pair the census no longer finds (fixed, or the source changed
 * shape) has outlived its subject and fails too, so the list can only
 * shrink.
 *
 * EMPTY ON THE DAY THIS LAW SHIPPED (20 Sep 2026). The one pair her own
 * words name most directly, `help-form-dialog.tsx`'s "Raised by" contact
 * `<Select>` and "Keep in the loop" `<StaffPillPicker mode="multi">", is
 * NOT a finding: the loop draws from a DERIVED staff roster (raiser +
 * admins + mentions, `workers/content/src/lib/stakeholders.ts`'s own
 * header, "a HYBRID model … the raiser + current team admins + everyone
 * @mentioned"), never a second read of the raised-by picker's own contacts
 * array, so the two do not share a source pool and there is nothing to
 * exclude — reasoned here rather than left for a reader to wonder why an
 * obvious-looking pair is not on this list. `web/components/tickets/*` is
 * owned by the tickets lane on this branch regardless, so a FUTURE change
 * that ever does point the loop at the same contacts array belongs to that
 * lane's own review, not a line added here pre-emptively for a finding the
 * census does not report today. */
export const MAIN_EXCLUDES_SECONDARY_EXEMPT: Record<string, string> = {}

// ── R93 (visual-accompanies-text) ───────────────────────────────────────────

/** R93 — a `<Select>`/`<RecordPicker>` choosing over a module-named source
 * array with no icon drawn (`icon=` missing on a `<SelectItem>`, or `icon:`
 * missing from a `<RecordPicker>`'s option literal). Keyed `${file path
 * relative to repo root}#${the source array expression feeding the block}`
 * — an EXPRESSION, never a line number. Rot-checked both ways: an entry
 * naming a block the census no longer finds (fixed, or the source changed
 * shape) has outlived its subject and fails too, so the list can only
 * shrink.
 *
 * ONE ENTRY ON THE DAY THIS LAW SHIPPED (20 Sep 2026): the agency ticket
 * form's own module `<RecordPicker>` (`help-form-dialog.tsx`) draws each
 * module's text `mark` but never its `icon` — the exact gap this law
 * closes — and the file is owned by the tickets lane on this branch
 * (`web/components/tickets/*`), off limits while it is mid-edit. The
 * portal's identical module `<Select>` (`raise-ticket-dialog.tsx`) was
 * fixed the same day this law shipped, wiring the icon through the kit's
 * `SelectItem` `icon` slot. */
export const VISUAL_ACCOMPANIES_TEXT_EXEMPT: Record<string, string> = {}

// ── R94 (chip-order) ────────────────────────────────────────────────────────

/** R94 — a chip row carrying both an id chip (`<RecordRef`) and a status
 * chip (`variant="status"` or `ticketStatusCell(`) within one reading
 * window, built without routing through `orderChips()`
 * (`shared/web/chip-order.ts`). Keyed `${file path relative to repo
 * root}#${the id chip's own value expression}` — an EXPRESSION, never a
 * line number. Rot-checked both ways: an entry naming a row the census no
 * longer finds (fixed, or the source changed shape) has outlived its
 * subject and fails too, so the list can only shrink.
 *
 * ONE ENTRY ON THE DAY THIS LAW SHIPPED (20 Sep 2026): the ticket detail's
 * own "Related stories" row (`help-detail.tsx`) draws id, then a title
 * link, then the story TYPE chip, then the STATUS chip last — status and
 * type both sit after the law's own #2/#3 positions, and the file is owned
 * by the tickets lane on this branch (`web/components/tickets/*`), off
 * limits while it is mid-edit. `shared/web/ticket-chips.tsx`'s `TicketChips`
 * and `web/components/accounts/contact-panels.tsx`'s ticket row (the one
 * live ordering DEFECT this law's census actually caught — type rendered
...
export const COUNT_REGISTER_EXEMPT: CountRegisterExempt[] = [
  ...
  {
    file: "web/components/work/logs-dashboard.tsx",
    why: "SETTLED, not pending, 24 Sep 2026 — Aurora, verbatim, reading the built Logs dashboard back: \"kpi need background\". The four figures were a bare column (the kit's stat register borrowed with the box left behind, which is what `accounts-dashboard.tsx` still draws); she asked for the paper, so they are real `<StatGrid>` tiles inside the kit's own `Card` now, part for part the same tile `effort-card.tsx` draws below — her own \"unless explicitly said\" clause, said. THREE OF THE FOUR ARE NOT COUNTS AT ALL (hours this week, hours today, and how many people logged nothing last week are a duration, a duration and a headcount-of-absence), which is the same distinction the Effort entry below draws for its duration and percentage figures; the fourth, records worked on, IS a count, and it is a dashboard's headline figure rather than a number beside a panel title with nowhere else to go. `pulse.tsx`/`agent-blocks.tsx`/`work-logs-panel.tsx` above are unrelated call sites and stay \"pending her word\" on their own.",
  },
  {
    file: "web/components/accounts/accounts-dashboard.tsx",
    why: "HER OWN EXPLICIT EXCEPTION, AND THE FIRST ONE THIS TABLE HOLDS. R97's text is \"unless explicitly said\", and every other line here says \"pending her word\" because nobody had said it. Aurora, 23 Sep 2026, of this exact tab: \"the cards kpi need some kind of background, like effort.\" She was looking at the shape R97 asks for — the same two type steps with no card — and asked for the card. So the three figures (active accounts, countries, median tenure) are `<StatGrid surface=\"bare\">` inside `<Card variant=\"default\">`, which is `effort-card.tsx`'s own treatment rather than a third one, and this row is the visible record that the exception was GRANTED here rather than assumed. It is a tone and not a box: `variant=\"default\"` is soft paper, so the kit's own §2.8 (\"by rule no borders nowhere in the kit\", her ruling the same day) is not in tension with it.",
  },
  {
    file: "web/components/work/effort-card.tsx",
    why: "SETTLED, not pending, 21 Sep 2026 — Aurora, verbatim: \"make the metrics cards inside the container, like in the metrics artifact you did for me!\" The shared Effort card's own three lines (Cycle time / Effort hours / Flow efficiency) are real `<StatGrid>` tiles now, the same primitive `work-logs-panel.tsx`'s own summary strip below already calls, rather than the hand-rolled `font-mono` grid this entry used to name. Still not a COUNT of things (how many tickets, stories, stakeholders) — R97's own named examples — they are duration/percentage figures, the same distinction this entry drew before the switch to `<StatGrid>`; her second sentence the same round, \"next to effort show the count of record, not the total hours (that has a metric on itself),\" is what moved the RECORD COUNT off this card entirely and onto the title's own count register instead — the exact register R97 asks for (`EmptyGatedPanel`'s `count` prop, the same `TicketSidePanel`/`CollectionHeading` shape 'Stakeholders 4' already renders through). Named here rather than removed because she has now explicitly asked for the three FIGURES as tiles, which is the 'unless explicitly said' clause her own ruling leaves open — pulse.tsx/agent-blocks.tsx/work-logs-panel.tsx above are unrelated call sites and stay 'pending her word' on their own.",
  },
]
...
export const BUTTON_SIZE_EXEMPT: ButtonSizeExempt[] = [
  {
    ...
    why: "K62, 21 Sep 2026: the live-sync pill's own Refresh action, drawn in the kit toast's dense light-wash register (shared/ui/components/sonner/sonner.tsx's ACTION recipe) rather than a toolbar, a page head, a card header or a form foot, so it follows the toast's own control height, not R98's default.",
  },
  {
  ...
  },
  {
    ...
    contains: "onClick={() => onSave(draft ?? \"\")}",
    // RE-KEYED 23 Sep 2026, same control, same reasoning. The meeting record's
    // one inline capture row used to save the NOTES; her ruling that day
    // removed the notes surface and made the AGENDA the open field in its
    // place (`MeetingAgendaSection`, meeting-detail.tsx), so the expression
    // this entry is keyed on moved with it. Keyed by the expression rather
    // than a line, as every entry in this table is.
    why: "An inline capture row (a compact add/edit affordance beside its own input, not routed through FormShell), not one of the four named surfaces. Pending her word on whether an inline capture row is a form foot.",
  },
  {
...
/** A still-open title-chip-above-the-heading finding, keyed by `{file,
 * contains}` - the offending `title={…}` attribute's own opening text, never
 * a line number. R100's second, 22 Sep 2026 census: a chip marker (`<Badge`,
 * `RecordRef`, `identityChips`) appearing before the first heading marker
 * (`clampRecordHeading(`, `<h1`, `<Title`, `<Headline`) inside a `title=`
 * attribute on `<RecordChrome`/`<RecordDetail`. Rot-checked both ways by
 * `web/test/head-actions-centred.test.ts`. Empty on the day this amendment
 * shipped: the one finding the sweep found (`record-chrome.tsx`'s own
 * `titleBlock`) was fixed via the kit's new `aboveTitle` slot, not exempted. */
export interface TitleChipAboveExempt {
  file: string
  contains: string
  why: string
}

export const TITLE_CHIP_ABOVE_EXEMPT: TitleChipAboveExempt[] = []
