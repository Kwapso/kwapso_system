...
export const COLOURED_CHIP_OK: Record<string, string> = {
  ...
  "web/components/work/stories-screen.tsx#MoscowChip":
    "the MoSCoW priority tag — Aurora, 20 Sep 2026, verbatim: \"Render the priority as a colored tag on each " +
    "story card\" (and let users filter and sort the backlog by it). Priority, not status, drawn by the same " +
    "`Badge variant=\"status\" dot={…}` shape `tasks-screen.tsx`'s own `PriorityChip` already takes for the one " +
    "other coloured-by-priority field this app carries.",
}
