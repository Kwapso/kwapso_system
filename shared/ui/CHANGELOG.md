...
**Six props, and not one of them is a new idea.** `activeId` / `defaultActiveId` / `onActiveChange` is the id-based controlled/uncontrolled trio `flowdetail.tsx` and `split.tsx` already write (`selectedId` / `defaultSelectedId` / `onSelect…`), resolved `rating.tsx`'s way: the prop being PRESENT hands the value to the call site, so `activeId={null}` is a real controlled answer ("nothing is active") and not a fall-through into the component's own state. `interactive` is `sankey.tsx`'s prop - same word, same default (on), same meaning. `figures` is the only new noun: `"always"` (the default) is chapter 18's own drawing, every row carrying its share at rest, so NO existing call site moves; `"active"` prints nothing until a segment is pointed at or tabbed to, which is her ruling. `formatSegment` is the row's whole readout for a caller with a sentence of its own.

**The legend row is the segment's control, because an SVG sector cannot hold a tab stop.** Each row is a real `<button>` and its own tab stop, carrying the segment's whole readout as its accessible name - `sankey.tsx`'s law, quoted in the file - so a screen reader is never told less than the pointer reveals, at rest or active. Pointer enter/leave and focus/blur all route through one `activate`. Pointing at the RING activates the same segment through `Pie`'s own `onMouseEnter`/`onMouseLeave`, so the sector and the row are one thing and not two controls.

**Nothing washes and nothing dims.** `chart.tsx`'s and `sankey.tsx`'s standing law: the ring is drawn identically at rest and active. The active ROW takes `--surface-quiet` (a swap between two defined tones, never a fade - `rating.tsx`'s own hover law) and, under `figures="active"`, its figures appear. The row's horizontal padding is paid back by an equal negative margin, `activity-feed.tsx`'s idiom, so the legend's dots do not move when a row lights up.

**`legend={false}` keeps the pointer and hands back the keyboard.** A caller drawing its own legend outside still gets `onActiveChange` from the ring and still drives the ring through `activeId`, so an outside legend and the ring stay in step; it owns the keyboard route the same way it owns the rows.

**Checks.** New: `components/donut/check-donut.mjs`, wired into `npm run check` (and `npm run check:donut`). Static pins for the resolution, the ring's handlers and the row's four; then the component MOUNTED through vite SSR + `react-dom/server` (`check-badge.mjs` §6's loader) for what is printed and when; then the state machine DRIVEN FOR REAL - `Donut.render` called inside a probe's own render, so a call to a captured handler is a render-phase update React answers by rendering again - for pointer enter, focus, pointer leave, blur, a ring sector's own enter, the controlled path and the no-double-report guard. Each section was proved by breaking what it guards: a no-op `onFocus` is caught by the driven section ALONE (the static pins stay green, which is the point of having it), a figure printed at rest by the mounted section, a missing ring handler and a `activeId != null` resolution by both. The measured counterpart is the book itself - `#/charts/circular` in `npm run dev`, where a real `mouseover` on `path.recharts-sector` was confirmed to reach the handler, and the active row's fill measured 6.66:1 for its label and 4.86:1 for its figure in light.

### Added - `FilterOverlay`, the one surface the collection toolbar's Filter control opens - 2026-09-23

Aurora's ruling of 2 Sep 2026 said the filters must open as *"a temporary overlay not a second row"*, and her ruling of 23 Sep 2026 chose between five drawn designs with three words: *"filter drop sheet popover"*. The recommendation she accepted said the two named forms are not two components: *"the sheet is simply a popover that ran out of room."*

**So this is one component with one primitive under it, and the two forms are two geometries of it.** `FilterOverlay` (`components/filter-bar/filter-bar.tsx`) takes the toolbar's own Filter control as a node (a `FilterBar` add slot, which keeps that pill's drawing where it already is) and opens a Radix popover from it. The POPOVER form anchors to the trigger and is 20rem wide. The DROP SHEET form anchors to the TOOLBAR through a virtual anchor and takes the toolbar's own width, so it falls across the full row and floats over the list. Nothing else differs between them.

**The form is decided by the content, never by a call site.** `filterOverlayForm(span, hasRoom)` is exported and pure: `span` is what the facets cost in field rows (`FACET_SPAN`: a closed field is 1, a range facet is 2), and a total over `FILTER_POPOVER_BUDGET` (3) is the sheet. A screen that had to choose would choose correctly once and be wrong the day a facet was added to it; a number derived from the facet declarations moves on its own.

**The phone gets a third geometry, and it is the kit's own.** Below 45rem neither form has room, and `sheet.tsx`'s `NARROW_BOTTOM` already states the house answer for every other overlay at that width: it rises from the bottom, full width, capped at 85dvh, with a grabber. So `useHasRoom()` false opens the kit's `Sheet` at `side="bottom"` with the identical body and foot. The JS media read that `sheet.tsx` refuses for its own geometry is safe here because nothing renders until the reader presses the trigger, which is after hydration.

**The foot carries Clear all and Show N**, the two controls the design draws. Show N is the live result count and the way out; it confirms nothing, because a facet applies the moment it is picked and there is no Apply step.

**`ToolbarRow`'s track now carries `data-filter-anchor`** (`FILTER_ANCHOR_ATTR`), which is how the drop sheet finds the box it spans. One attribute on the one row every collection toolbar is. A host that sets none degrades to the popover's width rather than failing.
...
**The cause.** `FOOTER_TWO_COLUMN_QUERY` (`components/record-detail/record-detail.tsx`) read `"@min-[calc(16.25rem*2_+_var(--space-7))]"`. A container query's condition is held to the same rule a media query's is: `var()` is a declaration-time substitution, resolved only once a rule has already matched, and a query's own condition is evaluated before any declaration is - so a custom property cannot appear inside one at all. CSS does not warn about this; the whole at-rule prelude is unparsable and the rule is dropped silently, which is why nothing anyone tried on the consuming side (a rebuild, a different width, a different container) could ever have surfaced it.

**The fix resolves the arithmetic to a literal.** `--space-7` is `2rem` (`foundations/tokens/tokens.css`), so `16.25rem * 2 + 2rem` is `34.5rem`; `FOOTER_TWO_COLUMN_QUERY` now reads `"@min-[34.5rem]"`, with the arithmetic written in the constant's own comment so the next reader can recompute it if either token moves - a query condition still cannot read a token live, only a literal.

**A second, independent bug, found while proving the first fix actually reaches the stylesheet.** Every call site read `` `${FOOTER_TWO_COLUMN_QUERY}:…` ``, interpolating the constant into a template literal. Tailwind's scanner reads source TEXT for a complete candidate class and never evaluates JS, so a class name assembled this way is invisible to it - the same limitation `tabs-view.tsx`'s own `TabsCount` restatement already documents ("Tailwind cannot resolve a class string assembled at runtime"). A local `vite build` proved it directly: with the `var()` already fixed but the interpolation left in place, none of the five classes (the two-track `grid-cols`, both `order-none`, both `col-start`, both `row-start`) reached the built CSS; spelling each one out as a literal `"@min-[34.5rem]:…"` string made all five appear, correctly gated under `@container (width>=34.5rem)`. `FOOTER_TWO_COLUMN_QUERY` survives as documentation - the one place the number is decided and its arithmetic explained - and every call site now writes the literal directly.

**Checks.** `foundations/rules/check-query-condition-var.mjs` is new, wired into `npm run check` (`npm run check:query-condition-var` standalone): every `@min-[…]`/`@max-[…]`/`[@container(…)]`/`[@media(…)]` condition under `components/` and `compositions/` must not carry a `var(` anywhere inside it, no exemption table (unlike a scoped law like the overflow-axis check, there is no legitimate reason a query condition needs a custom property). `compositions/templates/check-screen-shell.mjs`'s own footer-grid census (part of its existing air/bottom-edge check) is corrected the same way: its three assertions now read the LITERAL `"@min-[34.5rem]:…"` strings at each call site rather than the constant's name, since a check pinned to the constant would have stayed green through the exact interpolation bug it exists to catch. Both proved by breaking what they guard and watching them fail - the `var()` put back into `FOOTER_TWO_COLUMN_QUERY`, and separately the grid-cols call site reverted to the interpolated form - then restored from a `cp` copy each time, never `git checkout --`. `npm run check` is green end to end except the one pre-existing, unrelated contrast finding (`components/image/image.tsx` under `gallery.tsx`, surface-quiet on `--btn-disabled-fill`, ratio 1.000).

### Added - RecordDetail takes a row above the title, a sibling of Title rather than a passenger inside it - v1.2.158 - 2026-09-22

Aurora, verbatim, 22 Sep 2026, on a detail screen's head: *"on detail screens, the title buttons need to be alignes with the title! currently they are slightly abovem thats wrong."* Measured on the consuming app's staging build: the head actions row sat 17 to 18px above the title text's own optical centre, identically on every detail screen and both widths, while that app's own `head-actions-centred` law passed - true, because the row WAS centred on the heading box. The heading box was the bug.

**The cause.** `Title` (`components/title/title.tsx`) centres its `actions` cluster on the CROSS AXIS of its own root row (`items-center`, the 21 Sep 2026 ruling, CHANGELOG v1.2.146) - a two-item row, the eyebrow-plus-heading block on one side and `actions` on the other, so `items-center` measures whichever of the two is taller and centres both against it. That is correct for what `Title` was ever asked to hold. The consuming app's own record header wraps an identity-chip row (record number, status, parent-record chip) ABOVE the title - an owner ruling, 2026-09-01, "pills row, then title" - and this composition had no slot for a row above the title at all: override 73 (2026-08-26) had already moved this file's OWN identity row BELOW, and the one region drawn before the heading, `eyebrow`, is unused on that header (see `RecordChrome`'s own header comment, "no eyebrow anywhere in this block"). So the app folded its chip row INSIDE `title` itself, the only region left - which rendered correctly and broke the centring specifically: a `title` node carrying a chip row above the real heading text makes `Title`'s own heading slot taller than the heading alone, so `items-center` centres `actions` against the composite, 17 to 18px (a chip row's own height plus its bottom margin, halved) above the heading's true centre.

**The fix is a new slot, not a new rule.** `RecordDetail` (`components/record-detail/record-detail.tsx`) gains `aboveTitle`, rendered as a plain SIBLING immediately before `<Title>` rather than inside it - the identical shape `meta` already is for a row BELOW `Title`, mirrored upward. `Title`'s own box is therefore always the heading (and `eyebrow`, when one is given) alone, whether or not a caller uses `aboveTitle`, so `items-center` keeps measuring what it has always measured. Spacing is deliberately the caller's: `aboveTitle` carries no margin of its own, because the chip-row-to-title gap was itself a separate owner ruling with its own number ("from chips to title only 10"), not a value this file should decide. `RecordChrome` (`compositions/templates/record-chrome.tsx`) forwards the same prop straight through, undecorated - it composes its OWN below-title identity row from `recordNumber`/`collectionLabel`/`chips` exactly as override 73 left it, and does not compose `aboveTitle`'s content the same way, because unlike the row below, the row above is not this file's own concept to shape; it is a bare pass-through, the same as `hero` and `panel`.

**Nothing existing moves.** A caller that never passes `aboveTitle` renders the identical DOM `RecordDetail`/`RecordChrome` always have - the prop defaults to absent, `hasBand` treats it exactly as it treats every other optional region, and no existing specimen or check owns a pattern this changes.

`npm run check` was not run against this kit in isolation this pass - the consuming app's own lane owns proving the fix bites, against its OWN `head-actions-centred` law, by folding a chip row back inside `title` and watching that law's strengthened census fail, then restoring from a `cp` copy.

### Fixed - the record footer band lives in the shell's footer slot - v1.2.155 - 2026-09-22

Aurora, 21 Sep 2026, over the sixth screenshot: *"The latest activity always has to be at the very bottom"*, and, the same round: *"also, make footer not inside a container, but the full row side to side (withing the main content)"*. The sides were answered in v1.2.151 and v1.2.153. The BOTTOM was not: measured by paint on the consuming app's staging build the same day, a paper strip went on sitting under the band on every record page - 24px on the ticket page (115px at 760 tall), 62 to 198px on a knowledge source, flush on a story at desktop and 96px at 760.

**Three fixes chased it and all three were escapes.** A page container grown past the pane's own padding, retired the day it shipped: growing a box past a scrolling ancestor's content edge IS overflow, and Chromium then reserves that ancestor's `padding-bottom` a second time, so scrolling to the pane's own end dragged the band back up by exactly the amount it had been grown. A fixed negative bottom margin on the app's own footer wrapper, which reaches the pixel and is a number nobody can derive from anything. And v1.2.153's own move of `DENSITY_BODY` off the scroller onto `screen-shell-stack`, which was right for the band's sides and put the strip back under every band in the app, because the stack's trailing `py` then sat under all of them.

**The band's home is the shell's own footer slot now, and the shape is structure rather than arithmetic.** `ScreenShell`'s `footerNode` (`compositions/templates/screen-shell.tsx`) renders INSIDE the one scroller, `screen-shell-body`, so R91 holds and the band is still reached by scrolling to the end of the content ("you reache it like normal footer. this is a law", 19 Sep 2026), never pinned; and OUTSIDE `screen-shell-stack`, so `DENSITY_BODY`'s own inset never wraps it on any side. A new element between the two, `screen-shell-column`, is `flex min-h-full min-w-0 flex-col`: its `min-h-full` resolves because `screen-shell-body` is a flex item with a real used height, the stack carries `grow` so a screen nested in `children` still gets a real height to divide, and the footer wrapper carries `mt-auto` and no padding on any side. A short record's band therefore lands on the pane's own bottom edge and a long record's lands after its content, with nothing measured anywhere. The stack's own trailing `py` becomes the air ABOVE the band, which is the 24 the 21 Sep page already ruled.

**The band loses its escape in the slot, and keeps it everywhere else, through one property rather than a flag.** `BODY` now publishes `--pane-escape-x` beside `--pane-inset-x` at the same `--space-6`: the first answers "how far is this part from the pane's edges", the second "what does its own text owe so it lands under the h1". They were one property while the band only ever drew inside the padded stack. `RecordDetail`'s footer `Card` in the `band` register reads `-mx-[var(--pane-escape-x,var(--pane-inset-x,0px))]` for the escape and goes on reading `--pane-inset-x` for the padding it pays back inside itself; the footer slot's own wrapper rebinds `--pane-escape-x` to `0px`, so a band rendered through the slot pulls nothing (it already spans the pane) while its first word still lands under the h1. A caller that renders the band inside `children` - a demo cell, a dialog, a screen not yet moved to the slot - inherits the pane's own value and draws exactly as it has since 21 Sep. The `--radius-pane-edge` machinery is untouched and needs no gate: the pane already rebinds it to `0px`, and the slot is inside the pane.

**Checks.** `check-screen-shell.mjs` gains the footer-slot check: `screen-shell-column` reads `flex min-h-full min-w-0 flex-col`; `{footerNode}` renders after the stack's own `{children}` with a closing tag between them, so it is the column's last child and the stack's sibling, never the stack's child; the footer wrapper reads `mt-auto min-w-0 [--pane-escape-x:0px]` and carries no padding utility on any side. The existing air/bottom-edge check gains `BODY`'s own `--pane-escape-x` publication, and its two `record-detail.tsx` patterns (the escape, and the whole `footerRegister === "band"` ternary) move to the new property. `verify/ink-footer/page.tsx` gains `PaneFloorProof` and two cells that hand the band to the real `ScreenShell`'s `footer` slot and read `document.elementFromPoint` at the pane's own bottom edge minus two pixels - a SHORT record (the `mt-auto` case) and a LONG one scrolled to the pane's own `scrollHeight` (the case the retired grown-container fix lost). `npm run check` and `npm run build` are green end to end.

### Fixed - nested cards keep their insets inside a plain card; the empty register breathes - v1.2.154 - 2026-09-22

Two rulings, Aurora, 22 Sep 2026.

**A default card nested inside a plain card was losing its own insets.** Her words, on a screenshot of the ticket page's Effort section (three metric tiles, `Card variant="default"`, dropped inside the plain Effort `Card`): *"the contact is touching the border."* Each tile's label and figure sat flush against the tile's own left edge. The cause: `CardHeader`/`CardContent`/`CardFooter` (`components/card/card.tsx`) found their shell's `plain` variant with `group-data-[variant=plain]/card:px-0`, keyed off `Card`'s own `group/card` marker class. Tailwind compiles a named-group modifier as an ordinary CSS descendant selector, `:where(.group\/card[data-variant="plain"]) &`, which matches ANY ancestor carrying that class and attribute, not only the nearest one, and every `Card`, at every variant, carries `group/card`. A `default` metric tile nested inside the plain Effort card was itself wrapped in `group/card`, so its own parts matched the OUTER plain shell's `data-variant="plain"` straight past their own `default` shell in between, and lost their insets to a grandparent that was never their own parent.

**The fix moves detection from the DOM to React.** `Card` now provides its own resolved `variant` on a new `CardVariantContext`; `CardHeader`, `CardContent` and `CardFooter` each read it with `React.useContext` instead of the old `group-data-*` selector. React context resolution always returns the NEAREST enclosing provider, never an any-ancestor walk, so a nested `Card` shadows an outer one's variant for everything mounted inside IT and only inside it, the exact shape the CSS descendant match could not express. Context was chosen over a `[&[data-variant=plain]>&]` direct-parent selector because a part is not always a literal DOM child of `Card`'s own root: a composition may wrap it in a fragment or another layout node first, and a `>` combinator breaks the moment one exists, while `useContext` does not care what sits between the JSX elements. It was chosen over threading a `data-*` attribute through `React.cloneElement` because that would need every call site's children walked and cloned for no visible gain. `Card`'s own root still carries `group/card` and `data-variant`, unchanged: `kanban.tsx`'s `BoardCard` reads that exact pair on purpose, to detect a `plain` `Card` ancestor several levels up through a board that paints no ground of its own, and needs the CSS descendant match's breadth to do it, a different, still-correct consumer of the same marker, untouched by this fix.

**`check-card.mjs` gains the nesting proof.** Six structural assertions (`CardVariantContext` declared via `React.createContext`; `Card` renders `CardVariantContext.Provider` wrapping exactly `children`; all three parts call `React.useContext(CardVariantContext)`) plus a seventh that the old `group-data-[variant=plain]` string no longer appears in any of the three parts' own bodies, proved by running the check against the pre-fix source and watching all seven fail, then against the fix and watching it pass. `demo/sections/c-d.tsx`'s Card section gains a live nesting specimen: a plain Effort-shaped card with three default metric tiles inside it, each keeping its own padding.

**The plain empty register wanted the air the card used to give it.** Same day, a follow-up: after seeing v1.2.152's flush `CollectionRegister variant="plain"` sitting 10px under the toolbar with no air of its own, Aurora, verbatim: *"ok, but need a bit more spacing over it (like it was with the card)."* The card she means is the `block` register `plain` replaced; the figure restored is not `block`'s own 32px (`--space-7`) panel inset, it is `CardContent`'s own default vertical step before `lg:`, `py-6` (`CARD_CONTENT_INSET_Y_DEFAULT` in `card.tsx`, `--space-6`, 24px), the same rhythm the Effort tiles above spend on their own body. `registerVariants`' `plain` branch (`components/collection-frame/collection-frame.tsx`) now reads `"items-start pt-[var(--space-6)] pb-[var(--space-6)] text-start"`: flat top and bottom air, left edge still flush, still no fill. `check-collection-frame.mjs` gains a fifth assertion pinning the exact string, proved the same way against the pre-fix source. `demo/collections/ch-co.tsx`'s notes on the plain-panel registers are updated to state the new figure and where it comes from.

`npm run check` and `npm run build` are green end to end.

### Fixed - the scroller's clip box is the pane, so the footer band paints edge to edge - v1.2.153 - 2026-09-22

Aurora, 22 Sep 2026, the same words v1.2.151 already quoted and had believed it had answered: *"In the fourth screenshot, on the footer, there should be no white on the sides. Make the black go side to side."* v1.2.151 fixed the ARITHMETIC - `-mx-[var(--pane-inset-x,0px)]` was made to compute the right number, and `verify/ink-footer`'s `BandProof` confirmed it with `getBoundingClientRect()`, 0px gap at 1440 and 760, both PASS. What that proof could not see is that a `getBoundingClientRect()` gap of 0 is not the same claim as "the pixel paints": a box's own rect moves with its margin regardless of what clips it, so a band whose rect reaches the pane and whose PAINT does not would still read PASS on that proof. It did, live: the ancestor walk this entry ships with (Playwright against staging, storage-state auth, ticket T0001, `/tickets/01M1XVSTF6A2ZA16XSWTPB30MK`, four widths and both rail states) measured the footer band's own rect exactly equal to the pane's (`data-slot="screen-shell-content"`, left 163.95 / right 1424 at 1440) and the paint starting 24px inside it anyway - `elementFromPoint` at pane-left + 2px hit `screen-shell-body`, and the dark fill began at x 188.

**The ancestor, walked live, tag by tag, from the footer `Card` up to the pane.** Every ancestor's computed `overflow`/`overflow-x`/`overflow-y` and rect, printed rather than guessed: `record-detail`, a plain `min-w-0` wrapper, the app's `ticket-footer-band`, `ticket-detail-body`, two more plain flex wrappers, all `overflow: visible`, all already at the pane's own edges (163.95/1409) because the negative margin's ARITHMETIC was already correct. Then, inside `screen-shell-body` - the kit's own scroller, `overflow-y: auto` (which the CSS overflow spec forces the sibling axis away from `visible` too, so `overflow-x` there read `auto` even though the class only names `overflow-y-auto`) - one more app div, `mx-auto flex w-full max-w-none min-w-0 h-full flex-col overflow-x-clip pb-24 md:pb-0`, whose padding box starts at exactly 187.95 (188 rounded) because it carries no padding of its own and inherits the narrower box `screen-shell-body`'s own padding had made available to it. THAT is the clipper the live measurement names, byte-identical at 760 wide. It lives in `web/components/shell/app-shell.tsx` (`kwapso_system`, the consuming app, not this repository), added for a documented and unrelated reason - keeping the record header and tab strip's own `position: sticky` pinned to the true scrollport rather than silently becoming a second scroll container (that file's own comment, "the document stays the one scroller"). This repository cannot edit that file and does not attempt to; what it owns is its OWN link in the chain, and that link is now gone.

**`screen-shell-body`'s own clip box is now the pane, unconditionally, not by the coincidence of a zero border-width.** Before this entry `screen-shell-body` (the scroller) carried `DENSITY_BODY`'s own `px`/`py` directly - its padding box happened to equal its border box only because its border-width is zero, which made it LOOK like it was not contributing to the clip in this one measurement, but never made that a rule. `DENSITY_BODY[density]` (and the `trail && !band ? "pt-0 lg:pt-0"` gap it carries) moves one level in, onto `screen-shell-stack`, which is now ALWAYS rendered rather than only "when there is more than one thing in it" - a screen with neither `strip` nor `footerNode` still gets `children` alone at the identical inset, the stack being a `flex-col` with one child paints exactly as no flex context at all, so nothing a caller can see moves. `screen-shell-body` itself now reads `className={BODY}` alone, and `BODY`'s own literal carries no padding utility. A full-bleed part nested inside `children` therefore escapes ONE wrapper's padding (the stack's) to reach a scroller whose own box already IS the pane - which is what the kit's own reference composition (`compositions/templates/record-chrome.tsx`, `ScreenShell` wrapping `RecordDetail` directly, no intervening wrapper) now proves live, and what an app's OWN added wrapper - this repository's export or otherwise - can still defeat, because no CSS a descendant writes can escape an ancestor's non-visible overflow. That is not a gap in this fix; it is the fact this fix's own verify proof states honestly rather than papers over.

**Checks.** `check-screen-shell.mjs` gains "the scroller's clip box is the pane" - `screen-shell-body`'s own render site reads `className={BODY}` alone (no `DENSITY_BODY`, no bare padding utility), `BODY`'s own literal carries none either, and `screen-shell-stack`'s render site reads `DENSITY_BODY[density]` (so the inset is relocated, not dropped) - and the existing trail-spacing check's `pt-0 lg:pt-0` pattern moves from `screen-shell-body` to `screen-shell-stack` alongside the padding it zeroes. `verify/ink-footer/page.tsx` gains `ElementFromPointProof` (reads `document.elementFromPoint`, not `getBoundingClientRect` - the one API that reports what a pixel actually paints) and `ClippingScrollerProof`, two cells: `RealShell` (the real `ScreenShell` + `RecordDetail`, footer nested in `children` exactly as `RecordChrome` composes it) reads PASS - the paint now reaches the pane's edge; `HostileShell` (the identical tree with the live ancestor walk's own `overflow-x-clip` wrapper reproduced around the footer) reads PASS too, because its EXPECTED result is that the paint does NOT reach the edge - a documented, honest limit, not a bug this entry claims to have closed. `npm run check` and `npm run build` are green end to end.

Aurora, verbatim, on the Accounts / Inactive tab screenshot ("Nothing matched. Try fewer words, or clear the filters." drawn on a soft paper card): *"The empty collection now. We need to get rid of the card background."* The app side was already fixed - `CollectionEmptyState` stopped papering itself the same day - and this entry is the kit's own `CollectionFrame` composition catching up: v1.2.149 shipped a plain panel's empty, loading and failed registers as one rule ("the empty/loading/failed registers flip to variant block inside a plain frame, the one register that keeps its paper"), and every screen built on that composition - module automations, settings choices, internal screens, work panels, one stories view, one tasks tab, the sprints all view - inherited the paper along with it.

**The rule splits in two.** `registerVariants` (`components/collection-frame/collection-frame.tsx`) gains a third branch, `plain`: no fill, no radius, no inset of its own - `"items-start text-start"`, nothing else - so the register stands directly on whatever ground the panel already painted. `CollectionFrame` now computes two separate constants instead of the one `registerVariant` it used to share across all three body states: `registerVariant` (`panelSurface === "plain" ? "block" : "inline"`, unchanged) still drives the LOADING and FAILED registers, and a new `emptyRegisterVariant` (`panelSurface === "plain" ? "plain" : "inline"`) drives the EMPTY one alone. The empty register - and the identical register a filter reduces a collection to, since this frame carries one `empty` boolean and no second "filtered" flag - therefore sits with no card at the panel's own rhythm: 10px under the toolbar through the plain panel's own `gap-[var(--space-2h)]` column gap when a toolbar is drawn, or at the frame's own top edge through the panel's own `pt-0` when it is not. Neither number is spent a second time in the register itself.

**Loading and failed were left alone, on purpose.** Both still take `variant="block"`, the register that keeps its paper: a spinner or a failure with nothing under it reads as a page that broke rather than as a place that is empty, which was the 21 Sep reasoning for all three states and still holds for these two. Nothing here reaches for the kit's `Alert` (`components/alert/alert.tsx`, `bg-surface-lift`) - the brief was to keep whatever the existing law already draws, not invent a new one, and `block`'s soft paper was already the law for these two states before this entry.

**`panel="paper"` is untouched.** Every register on a paper panel keeps `variant="inline"`, exactly as ruling J2 always drew it - the frame itself is the paper there, so none of the three registers needed a card of their own in the first place.

**Checks.** `components/collection-frame/check-collection-frame.mjs` is new and pins four things: `registerVariants`' `plain` branch stays exactly `"items-start text-start"`; `registerVariant` and `emptyRegisterVariant` are computed as two separate constants rather than one shared one; the empty register reads `emptyRegisterVariant`; and the loading and failed registers still read the unchanged `registerVariant`. Every one of the four assertions was proved by breaking exactly what it guards and watching the check fail. `demo/collections/ch-co.tsx`'s `CollectionFrame` section gains the third `CollectionRegister` variant to its own specimen row, a relabelled empty/loading/error panel stating the split, a no-toolbar empty example proving the frame's-own-top case, and a `panel="paper"` comparison proving every register there is unchanged. `npm run check` and `npm run build` are green end to end.

### Fixed - footer band side to side at every width, stripe register, one column order - v1.2.151 - 2026-09-22

The client, verbatim, on the footer band's fourth screenshot (a ticket page, rail expanded, wide window): *"In the fourth screenshot, on the footer, there should be no white on the sides. Make the black go side to side."* and *"The latest activity always has to be at the very bottom, and also make it a stripe, not a container. On the other hand, in any kind of screen that requires that the footer displays only one column instead of two, put the record on top and the latest activity on the bottom."*

**The inset audit.** `screen-shell.tsx`'s `BODY` publishes `--pane-inset-x` as the flat `var(--space-6)`, and `SHELL_CONTENT_INSET_X`/`DENSITY_BODY` spend the identical flat `px-[var(--space-6)]` - no breakpoint, no rail state, no scale step diverges either side. Measured live (`verify/ink-footer`'s new `BandProof`, a mock pane with the same two published properties BODY carries, at 1440 and 760) and against the real `ScreenShell` + `RecordDetail` composition in the demo book (rail expanded and collapsed, 760 through 1991): the band's edges reach the pane's own edges at every one of them, 0px gap. The screenshot's gap is therefore a defect of an OLDER kit tag - this file's own `--pane-inset-x` mechanism did not exist before 21 Sep 2026 (v1.2.149) - not of what ships here; `check-screen-shell.mjs` gains a pane-inset-x publisher-parity check (walks every breakpoint-prefixed `px-` variant `SHELL_CONTENT_INSET_X`/`DENSITY_BODY` spend against every variant `BODY` publishes, and fails if the two sets of prefixes ever disagree) so a FUTURE breakpoint added to one side alone cannot reopen it silently.

**The stripe register.** `RecordDetail` takes `footerRegister?: "band" | "stripe"`, default `"band"` (byte-identical to every existing call site). `"band"` is CH27.8's card exactly as it already drew: `-mx-[var(--pane-inset-x,0px)]` and `rounded-[var(--radius-pane-edge)]`, gated together so neither drifts from the other. `"stripe"` is for a sheet or another narrow host that publishes no `--pane-inset-x` at all: no margin (nothing to escape), `rounded-none` unconditionally (not `--radius-pane-edge`'s own fallback to `--radius`) - edge to edge of whatever box it is given, flush with that host's own last child by the host's own layout, the same way `CARD_FLUSH` is the shell's column deciding a flush edge, not the card.

**One column, either register: Record on top, Latest activity last.** The footer's two-column grid (`CardContent`) drops `auto-fit` for an explicit container query, `FOOTER_TWO_COLUMN_QUERY` (`@min-[calc(16.25rem*2_+_var(--space-7))]`, the identical arithmetic `auto-fit` used to compute implicitly), because `auto-fit` alone answers "how many columns" but leaves both columns in DOM order however many it draws - right for two columns (CH27.8's Activity-left/Record-right, unchanged) and wrong for one (this ruling's Record-top/Activity-bottom). Gated on `footerHasTwoColumns` (`showRecordColumn && showActivityColumn`, so a lone column at a wide container still spans full width rather than jumping to an explicit empty-sided cell): below the query, `footer-activity` reads `order-2` and `footer-record` reads `order-1`; at or above it, both take an explicit `col-start`/`row-start` instead, restoring CH27.8's own left/right cells regardless of `order`. `@container` is `CardContent`'s own - the kit's second container query after `toolbar-row.tsx`'s, for the identical reason: a stripe can stand in a sheet narrower than the viewport, and the column count has to answer to its own box, not the window.

`check-screen-shell.mjs` pins the register gate, the two-column query constant, and both regions' order/placement; `verify/ink-footer` gains `MockPane`/`MockSheet` and `BandProof`/`StripeProof`/`OrderProof`, which measure (not screenshot) a band at 1440 and 760, a stripe at 420, and the Record-over-Activity order at both - all five PASS, both palettes.
