...
| `--surface-record-footer` | → `--surface-inverse` | → `--kw-unlit-page` | CH27.8's ink footer, the **one block that is a different kind of surface in each palette**: an inverse panel in light, a RECESSED band in dark. It wears **no edge** in either — client, 2026-09-06. **Dark corrected 7 Sep 2026:** this was `--surface-raised`, which is `--card`, which is what the record detail's own card already is — so the footer painted itself its parent's colour at **1.000** and the client reported she could not see it. The rule both palettes now keep is that the footer is the record's DARKEST band: 17.386 in light, and in dark it drops below the card at **1.198** (plus its hairline) instead of matching it. |
| `--surface-record-footer-well` | `#3A3833` | *same* | The well a field or a pill sits in **on** that footer. One value in both palettes, because the footer is a dark surface in both. Measures 1.499 on the light footer and **1.587** on the dark one — the dark figure was 1.324 until the footer itself was corrected on 7 Sep 2026, and the well got MORE separation out of that fix, not less. |
