...
| 7.5 | A contact's page is its own page: no sprints, no rates, no contacts of its own | **DONE**, proved with Marta, who was a contact at two companies at the time. That case was why it stayed one table; see 15.1, the premise changed 22 Sep 2026 |
...
| 9.11 | The Sprints main page is killed; Waves leads Build | **DONE**, client ruling, 15 Sep 2026, verbatim: "sprints go inside waves. I would suggest killing the sprints main page completely and just keeping the waves one on top of the build section on the sidebar." Sprints' sidebar row, its `/sprints` top-level route (`web/app/sprints/[[...rest]]/page.tsx`) and the collection screen's render branch are all gone; the module and its permission right are unchanged, so Settings › Modules and the roles matrix still list it. A sprint is planned and opened from inside its wave now — wave-detail.tsx's own Sprints tab lists them and its "Plan a sprint" button is the only door that creates one — and its crumb trail is Wave → Sprint, never a Sprints rung. Waves moves from last in Build to first. See UI-RULEBOOK.md K21 and `shared/rules/registry.ts`'s `SECTION_HOSTED_ELSEWHERE.sprints` |
...
| # | The thing | Why not |
|---|---|---|
| 15.1 | Splitting companies and contacts into two database tables | It would cap a person at one company. Marta was a contact at two. The screens split completely either way, which is what Aurora actually asked for. **Since overtaken, 22 Sep 2026:** Aurora ruled a person can only be at one company, so the cap this row was written to avoid is now the rule itself; see DATA-MODEL.md § accounts + account_links + portal_users for the evidence and the SHAPE fix the table-split session owns |
