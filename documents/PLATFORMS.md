...
|---|--------|-------------------------------|----------------------------|
| 1 | **Per-team data isolation** | one **D1** (SQLite) database *per team* + one core D1 for global identity/billing | `shared/workers/d1-rest.ts` (`d1Query` / `d1ExecScript` / `d1QueryAcross` / `sqlString`) for TEAM data — plus 156 raw `env.DB.prepare(…)` sites in 29 files for the CORE database, which has no adapter (see below) |
...
Cloudflare's raw D1 binding API — at **156 call sites across 29 production files** —
...
mechanical rather than clever (one adapter, then a codemod over 156 sites), but
...
156 in 29 — and neither drift was visible to anybody reading the sentence. It was
