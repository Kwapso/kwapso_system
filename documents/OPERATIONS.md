...
> **`wrangler d1 export --remote` IS AN OUTAGE, AND IT WILL NOT ASK YOU IN A
> SCRIPT.** It warns that the database "will be unavailable to serve queries"
...
- **THE MAINTENANCE DOORS ARE THROTTLED, and a refusal is a 429, not a broken key.** ADDED 6 Sep 2026. The agency gateway forwards `/api/tenancy/*` and `/api/data-ops/*` BY PREFIX, which publishes all eight `x-admin-key` doors to the public internet — with, until this date, no throttle, no lockout and no record that anybody had ever tried a key. `guardMaintenance` (`workers/gateway/src/index.ts`) now allows **12 maintenance calls a minute per source address** and answers `429 too_many_requests` past that; every refusal writes a row to the central error store naming the address, so repeated rows there mean somebody is guessing `ADMIN_KEY` — **rotate it and check the address**. The ceiling is sized off real use: BOOTSTRAP's runbook makes two calls, OPERATIONS' error-log read makes one, and the seed/smoke scripts make one `create-team` each, so no honest caller comes near it. If a legitimate batch ever does, wait a minute rather than raising the number.
  It rides the `MAINTENANCE_LIMIT` **ratelimit binding** in `workers/gateway/wrangler.jsonc` (namespace 1105, both environments — its own namespace, so it never shares a bucket with the per-caller `CALLER_LIMIT` budgets, which count a different thing and key on a resolved caller rather than an address). **It fails OPEN when the binding is absent**, deliberately, so a broken limiter is never an outage and the code survives the window between a deploy and its binding — which also means deleting the binding silently disables the throttle. `workers/gateway/test/maintenance-limit-is-deployed.test.ts` is what stops that: it fails the build if `MAINTENANCE_LIMIT` is missing from EITHER environment, or if the code stops reading it. The KEY itself still fails closed — an unset `ADMIN_KEY` is a 503, and a wrong one a 403, as they always were.
...
- `kwapso-internal-media` + `kwapso-internal-media-staging`, the agency's OWN files: brand assets, staff photos (bound `INTERNAL_MEDIA` on content). ADDED 2026-08-12 with the agency-internal modules; this list omitted it, so BOOTSTRAP §3 created eight buckets while this page named six. Certificate files sat in this bucket too until the certificate module was killed whole on 14 Sep 2026 — the bytes already written stay, unreclaimed, under each team's `staff/` prefix (DATA-MODEL.md's `staff_profiles` entry says why).
...
- **`AGENT_FREE_DAILY=0` means zero free AI**, not "fall back to 50". Every
  numeric var behaves that way now (`numberVar`), on every environment — the
  old exception, `AGENT_NO_DAILY_CAP`, which once left staging's cap
  unenforced whatever the number said, is gone from every spender's config
  (see the `AGENT_FREE_DAILY` entry above).
...
- deploy:staging ends with **two** live smokes, both must pass or the deploy is considered failed:
  ...
  - `scripts/smoke-portal.mjs` (`npm run smoke:portal`) — the CLIENT front door, added 19 Aug 2026 because until then **half the product deployed unverified on every push**: the agency smoke never touched `kwapso-portal-staging`. It signs a client in at the portal's own hostname, knocks on **every** door in the gateway's `PORTAL_DOORS` allow-list (the census is derived from the table and from what the run actually called, so a door opened tomorrow is an untested door today and the run goes red), then attacks each of them with a second company's ids — list, search, by-id and by-write — with every negative's BAIT proved to exist first from the account that should see it. It also proves the two bugs of that week stay fixed: a deliverable whose visibility is revoked stops returning its file URL (grepped out of the whole response body, not read off one field), and — until 10 Sep 2026 — that no door reading the agency's own internal cost was reachable at the client's hostname. That was Law R24: the doors were derived from the one library those figures lived in, and each was asked twice, 404 at the client's hostname, 403 `client_login` at ours, to the same person. **R24 was retired on 10 Sep 2026 with the internal rates it fenced, and that clause of the smoke was retired with it.**
...
- `verify/` holds throwaway visual rigs — side-by-sides that mount the app's
  REAL components to settle one design decision from a picture, never from a
  paragraph. It is not a shipped surface and carries nothing this page's
  deploy chain touches: no worker, no build step, no entry in `npm run check`.
  The convention (the folder shape, when a `.claude/launch.json` entry is
  worth adding, why a rig is deleted the day its ruling lands, why `npm run
  check` never type-checks one) is written down once, in `verify/README.md`,
  not repeated here — this page ships the app; that one manages a directory
  that is never meant to.
