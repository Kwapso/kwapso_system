...
1. **One `callInternal(path, {cookie, timeout})` seam** (`shared/workers/`). Kills THREE findings at once: the cookie-forward internal-fetch dance is copy-pasted 5+ times (DRY); ~~**no `fetch` has an `AbortSignal`** anywhere, the D1 REST door (`cf()`), cross-worker calls, and the agent's model calls all lack a timeout, so one hung socket stalls a whole worker (resilience)~~ **— that half is CLOSED: R11 makes every external `fetch()` carry an `AbortSignal` timeout and a check enforces it, so the timeout is no longer an argument for the seam —** and the forward executors flatten 403/409/500 into one generic string (status preservation; also the cause of the agent's "stuck pending step"). **Two findings now, not three**, which is worth knowing before calling it the highest-leverage single change.
...
5. ~~**`d1QueryAcross` uses `Promise.all`** → one slow/failed shard fails an entire split-module read. Use `allSettled` + record the degraded shard.~~ **DONE 2026-09-07** — it is `Promise.allSettled` now, and the half of the ask that was left out was left out on purpose: it still fails LOUD on any rejected shard, because a sharded read that quietly dropped a shard's rows would under-report a count. What it gained is that the error NAMES which shards failed (`n/m shard(s) failed (…)`) instead of throwing the first raw one and hiding the rest. Tolerating a degraded shard is a per-query opt-in nobody has needed, and the comment at `shared/workers/d1-rest.ts` says so.
