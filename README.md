...
- **The agency app** (`web/`) is where Kwapso's staff work — ticket triage,
  ...
  money, the knowledge base, and an in-app AI agent at their side.
...
money — no rate card at all since 10 Sep 2026, when the client retired all
three in two rulings an hour apart; what is left on this worker is one read,
what an app gives back priced); **realtime**; **content** (tickets and
...
default 25 in `shared/workers/credits.ts`, and a real, different ceiling per
environment — read it off each spender's own `wrangler.jsonc` (`tenancy`,
`content`, `data-ops`) rather than trust a figure here, since the two are
meant to diverge on purpose. The old escape hatch, `AGENT_NO_DAILY_CAP`, is
gone from every spender's config — the `content` worker's carries the date it
left staging) plus a purchasable balance.
...
(throwaway visual rigs that settle a design decision from a picture rather than
prose, one per question, deleted once its ruling is recorded in the folder's one
tracked file, `verify/decisions.html` — the convention is `verify/README.md`),
[`design/`](design/) (the design-system
...
- **The Laws of the Base** (R1 to R99), [RULES.md](RULES.md), *machine-checked*: pinned to `shared/rules/registry.ts` and enforced by tests that read the source off disk (`web/test/rules.test.ts`, the per-worker `publish-seam.test.ts` for live-sync R1, the `gating-seam` suites, incl. the external mcp surface, for R10, `fetch-timeout` R11, `cron-records` R12, plus the scale/safety round: R13 self-healing catalog, R14 bounded lists, R15 live listeners, R16 exact counts, R17 idempotent transitions, R18 cross-module activity gating, R19 agent/MCP filter parity, R20 scanned boundary validation, R21 no agency door for a client login, R22 agent/MCP body-field parity, R26 the vector index narrows and the team's database decides, R27 described contracts, every backticked identifier in a tool description names something real, R28 the translation catalogue is exactly the set of strings the app says: a sentence missing from it ships untranslated, and an entry nothing says any more is an orphan, R29 the page has one width per front door and a screen never sets its own, R38 a record detail reads its record BY ID and never finds it in a page of a growing collection, R40 a stored file must reach a person: every door that puts bytes in a bucket is walked through to the screen that renders the reference, and a field that only ever reaches a form is not a read, R41 a file somebody picked is either sent or refused and never dropped: every create call site of a dialog that defers an upload must hand back the id its files hang on, R42 every accepted source type resolves to a declared reader on EVERY door or to an honest refusal, and no door chooses its own: one table both the upload door and the Drive lane ask, so the same PDF cannot read properly through one and come out as gibberish through the other, R43 agent/MCP tool-SET parity: a tool that exists on the agent's own catalog exists on MCP's too, or the gap is a named, reasoned line, and the reverse — R19/R22 prove a door has a tool on some surface, this proves the two surfaces agree with each other, R45 every kit composition is decided: all 47 files under `shared/ui/compositions/` are either a direct import this app actually reaches or a reasoned, rot-checked exemption naming why not — a composition nobody looked at, or hand-rolled screen UI that duplicates one, is the only unacceptable result, R46 every kit component and foundation is decided too: every component under `components/` plus the 3 foundations (icons, tokens, motion) — the count is derived by `kitInventory()` in `scripts/kit-coverage.mjs` and deliberately not written down here — is either REACHED — a direct import, or through another adopted part, or through a CSS `@import` a JS-only census cannot see — or a reasoned, rot-checked exemption, R60 an image FILLS its box: every picture either front door draws is `object-cover` and never `object-contain` — nor the same result reached by handing the kit's `Image` a `fit="contain"`, which is the second census and the one that caught a fit a class-only scan had missed; the vendored kit's own are out of the requirement and pinned in a count that may only fall (`KIT_CONTAIN_CEILING`, `web/test/an-image-fills.test.ts` — down to one, from five, after kit v1.2.73 adopted the ruling upstream), and the one exemption is a ticket attachment's preview, where the picture IS the content rather than a mark standing for a record whose name is written beside it), R61 a module's settings have two doors and one derivation: the client asked for a gear on each module's own screen *and* a Modules tab in Settings to "find the module once", so the gear, the full-screen page and the tab's index all ask ONE expression — every module in `MODULE_SETTINGS` has exactly one gear and every gear names a module that table declares, the Modules panel derives its rows instead of listing them, and the permission is written in a single place so a row can never be offered to somebody the page would refuse), R64 a section on the team area's strip has a door, or names the screen that took its place: the app's one entrance to that area is the derived "This team" list on Settings › Team, so taking a section OFF that list is the act that has to say where its material went — a rot-checked `SECTION_HOSTED_ELSEWHERE` line naming the reachable screen, and that screen must really make the door call each of the section's own acts dispatches, because naming a host proves nothing on its own; earned by three administrative acts on a team's people (change a role, remove a member, revoke an invitation) shipping green with no in-app path to any of them), R88 empty-state single door (when a section is empty, the empty state's "Add the first" is the only door), R89 footer on the edge (a record screen's footer sits at the screen's own bottom edge), R91 no nested scroll (one page scroll only, no nested scroll regions inside a page), R92 main excludes secondary (a chosen main person disappears from the secondary picker over the same pool), R93 visual accompanies text (a module's own icon rides beside its name everywhere it is chosen, filtered or viewed), R94 chip order (id, status, type, main parent, secondary parent — always this order, through the one `orderChips()` seam), R95 no em dash (never in the UI, an email or the glossary — no exemptions table), R96 the id chip is black (a record's own reference draws only through `RecordRef`, the shared ink chip register, never a hand rolled badge in another tone or bare text on a standalone id column), R97 a count never gets its own card (a number that counts things sits beside its panel's own title, in the count register, never its own card, tile or stat box, unless she has named the exception), R98 every button is the kit's own height (a button in a toolbar, a page head, a card header or a form foot is never `size="sm"` or a custom height/padding class), R99 no record closes while its own clock is still running (a story's Done, a ticket's resolve and a ticket's archive all refuse while a timer on the record is still running, through the one shared `refuseWhileTimerRuns`, and the closing button mirrors the same refusal). Break one → the build goes red. Adding a Law requires the rule, the registry entry, and a check, all three.  **And the check must be able to fail:** every source-scan strips comments before matching (this repo's comments discuss the very seams being scanned), matches a CALL not a word, boundaries each identifier, knows both export shapes, and carries a tripwire asserting it matched something. See CONVENTIONS.md § *Reading config, and writing a check that can fail*, each of those rules was earned by a check that passed its own sabotage.
- **The working agreement**, [WORKING-AGREEMENT.md](documents/WORKING-AGREEMENT.md): the operational rules — lane discipline, deploy order, ship gate, credentials, team migrations, kit tags, artifacts. Moved out of CLAUDE.md on the lean_foundation pass so CLAUDE.md can stay an index; this is the only copy.
- **How code here is written, in detail**, [BUILD-STYLE.md](documents/BUILD-STYLE.md): worker/handler shape, the screen engine, the vendored design kit, component folders, voice, the action-icon mapping. Same move, same reason.
...
- **How a screen is arranged**, [UI-RULEBOOK.md](documents/UI-RULEBOOK.md): the layer above UI-CONVENTIONS. A *rearrangement* rule book, expressible with the components `shared/ui/` already ships and the tokens the theme already defines, so every rule in it can be applied from `web/`, `web-portal/` and `shared/` without changing a component. Split by topic on the lean_foundation pass: this file is now a short index (title, contents, the rule-count table); the 281 rulings themselves live under `documents/ui-rulebook/`, one topic per file, in reading order.
...
- **The security model in one place**, [SECURITY.md](documents/SECURITY.md): trust boundaries, identity/sessions, the permission spine, tenant/account isolation (incl. the client-portal fence and the knowledge base's vector namespace), the AI agent's scope, the MCP token model, boundary validation, file uploads, secrets handling, and external-call safety — each claim naming the law, test, or DB constraint that enforces it.
