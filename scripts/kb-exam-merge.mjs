...
const KEY_CONFIDENCE_NOTES = {
  ...
  O3: 'RE-KEYED 2026-09-12: the two originally-keyed transcripts ("HORST errors", "HORST x claude", both 13 Aug) do NOT contain matching rules — read directly, one is a bug-fix debugging session (post-submit action reassignment), the other an API-integration/cost discussion. Neither names a single rule. The real material is diffuse: 13 individual "Extra ticket raised by HOGO about HORST" tickets, each raising ONE matching criterion/threshold/rule (exclusion criteria, language-level matching, score-over-90 notification, priority assignment, pendulum radius, and others) — confirmed by reading each, and this row\'s own `synth` tag already says a synthesis across several sources was expected. Keyed to all 13; `grade()`\'s `sourceIds.some(...)` already expresses "several sources legitimately answer this" without any change to the harness — the design was never the gap, three rushed keyings in one night were.',
}
